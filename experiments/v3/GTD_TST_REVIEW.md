...
- 当前审核周期集中修复：旧TG `+0.8H`才触发TRY-020，`+1H`也只等待匹配比较；150次teacher refresh逐次记录model/package SHA与完整target证据；checkpoint新增teacher、batch/CPU/CUDA RNG及显式resume；补齐tie、同向退化、非连续global/ZS和真实正target梯度隔离测试。
- 本地证据：`tests/test_gtd_tst.py` 7项通过；完整测试`531 passed, 2 warnings, 3 subtests passed`。warnings来自既有SCCC/V1测试，与GTD无关。
- Round 1：`pending_on_4ccf06d`
- Round 2：`pending`
