...
def test_ctpm_config_identity_is_parseable():
    path = Path("config/tries/v6_try_010_r3_pfde.yaml")
    config = yaml.safe_load(path.read_text(encoding="utf-8"))
    assert config["schema_version"] == "gzsl-paper.v6-pfde-train.v1"
    assert config["experiment_id"] == "V6-TRY-010-R3"
    assert config["rescue_of_experiment_id"] == "V6-TRY-010"
    assert config["rescue_index"] == 3
    assert config["total_updates"] == 28228
    assert config["eval_interval_steps"] == 141
    assert config["required_module_delta_h"] == 1.0
    assert config["semantic_parent_metrics_percent"]["H"] == 68.750566
