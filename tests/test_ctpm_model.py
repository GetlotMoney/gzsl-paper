...
def test_ctpm_forward_shapes_and_semantic_pair_contract():
    torch.manual_seed(1)
    model = _model()
    ...
    parent = model(images, patches, enable_s=False, enable_v=False, enable_i=False)
...
def test_ctpm_pair_correction_is_antisymmetric():
    ...
    delta = out.logits - out.base_logits
