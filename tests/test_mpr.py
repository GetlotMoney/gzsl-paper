...
from model.candidates.v2.trainers.train_mpr import load_config
...
def test_mpr_config_and_boundary():
    config,_=load_config(ROOT/"config/tries/v2_try_046_mpr_seed7.yaml"); assert config["idea_id"]=="IDEA-014"; source=(ROOT/"model/candidates/v2/trainers/train_mpr.py").read_text(encoding="utf-8"); assert source.index("for epoch in range")<source.index("# official test严格在MPR训练结束后加载。")
