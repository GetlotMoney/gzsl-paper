...
from model.candidates.v2.trainers.train_ara import load_config
...
def test_ara_config_and_boundary():
    config,_=load_config(ROOT/"config/tries/v2_try_092_ara_seed17.yaml"); assert config["attempt_id"]=="V2-TRY-092" and config["ridge"]==0.01; source=(ROOT/"model/candidates/v2/trainers/train_ara.py").read_text(encoding="utf-8"); assert 'unseen_images_used_for_gradient":False' in source
...
def test_cra_standard_output_contract():
    config,_=load_config(ROOT/"config/tries/v2_try_111_cra_contract_seed17.yaml"); assert config["attempt_id"]=="V2-TRY-111"; source=(ROOT/"model/candidates/v2/trainers/train_ara.py").read_text(encoding="utf-8"); assert 'model_best.pth' in source and 'checkpoint_last.pth' in source and 'data_fingerprints.json' in source
def test_cra_resume_config_and_checkpoint_contract():
    config,_=load_config(ROOT/"config/tries/v2_try_112_cra_resume_seed17.yaml"); assert config["attempt_id"]=="V2-TRY-112"; source=(ROOT/"model/candidates/v2/trainers/train_ara.py").read_text(encoding="utf-8"); assert '--resume-from' in source and 'optimizer_state_dict' in source and 'generator_state' in source and 'config_sha256' in source
