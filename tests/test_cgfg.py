...
from model.innovations.train_cgfg import load_config
...
def test_cgfg_config_and_boundary():
    config,_=load_config(ROOT/"config/tries/v2_try_076_cgfg_seed7.yaml"); assert config["idea_id"]=="IDEA-024"; source=(ROOT/"model/innovations/train_cgfg.py").read_text(encoding="utf-8"); assert source.index("for epoch in range")<source.index("# official test严格在CGFG训练结束后加载。")
