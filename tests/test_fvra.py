...
from model.innovations.train_fvra import load_config
...
def test_fvra_config_and_boundary():
    config,_=load_config(ROOT/"config/tries/v2_try_068_fvra_seed7.yaml"); assert config["idea_id"]=="IDEA-019"; source=(ROOT/"model/innovations/train_fvra.py").read_text(encoding="utf-8"); assert source.index("for epoch in range")<source.index("# official test严格在FVRA训练结束后加载。")
