...
from model.candidates.v2.trainers.train_jbec import load_config
...
def test_jbec_config_and_boundary():
    config,_=load_config(ROOT/"config/tries/v2_try_131_jbec_seed17.yaml"); assert config["attempt_id"]=="V2-TRY-131" and config["max_beta_residual"]==2.0 and config["max_gamma_residual"]==0.05; source=(ROOT/"model/candidates/v2/trainers/train_jbec.py").read_text(encoding="utf-8"); assert 'unseen_images_used_for_gradient":False' in source
