...
from model.candidates.v2.trainers.train_svpg import load_config
...
def test_svpg_config_and_boundary():
    config,_=load_config(ROOT/"config/tries/v2_try_053_svpg_seed7.yaml"); assert config["idea_id"]=="IDEA-016"; source=(ROOT/"model/candidates/v2/trainers/train_svpg.py").read_text(encoding="utf-8"); assert source.index("for epoch in range")<source.index("# official test严格在SVPG训练结束后加载。")
