...
from model.candidates.v2.trainers.train_vpa import load_config
...
def test_vpa_config_and_boundary():
    config,_=load_config(ROOT/"config/tries/v2_try_118_vpa_seed17.yaml"); assert config["attempt_id"]=="V2-TRY-118" and config["reverse_ridge"]==0.01; source=(ROOT/"model/candidates/v2/trainers/train_vpa.py").read_text(encoding="utf-8"); assert 'unseen_images_used_for_gradient":False' in source
