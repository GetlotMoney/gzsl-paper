...
class RGRAAssetTest(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = tempfile.TemporaryDirectory()
        self.root = Path(self.tmp.name)
        self.spec = RGRADatasetSpec(
            dataset="CUB",
            class_count=4,
            seen_count=2,
            train_count=3,
    ...
    def _write_visual_asset(self) -> None:
        ...
        outputs = {
            ...
            "test_seen_labels.pt": self._save_tensor(
                "test_seen_labels.pt", self.test_seen_labels
            ),
            ...
            "test_unseen_labels.pt": self._save_tensor(
                "test_unseen_labels.pt", self.test_unseen_labels
            ),
            "role_sentence_embeds.pt": self._save_tensor(
                "role_sentence_embeds.pt", self.role_sentence_embeds
            ),
    ...
    def _write_relation_asset(self) -> None:
        ...
        outputs = {
            "relation_texts.json": self._write_relation_output(
                "relation_texts.json", relation_texts
            ),
            "relation_sentence_embeds.pt": self._write_relation_output(
                "relation_sentence_embeds.pt", self.relation_embeds
            ),
    ...
    def _write_checkpoint(self) -> None:
        ...
        p_v5 = F.normalize(
            torch.randn(self.spec.class_count, EMBED_DIM, generator=generator), dim=-1
        )
        reader = {
    ...
    def test_train_loader_keeps_official_test_assets_unopened(self):
        missing_test_path = self.visual_dir / "test_seen_features.pt"
        missing_test_path.unlink()

        assets = load_rgra_train_assets(self.config, spec=self.spec)
    ...
    def test_eval_loader_opens_test_patches_and_not_checkpoint_or_train_split(self):
        ...
        train_path = self.visual_dir / "train_features.pt"
        train_path.unlink()

        assets = load_rgra_eval_assets(self.config, spec=self.spec)
    ...
    def test_relation_manifest_must_bind_visual_manifest_sha(self):
        ...
        self.relation_manifest.write_text(
            json.dumps(relation_manifest, indent=2) + "\n", encoding="utf-8"
        )
        ...
        with self.assertRaisesRegex(ValueError, "relation asset identity"):
            load_rgra_train_assets(self.config, spec=self.spec)
