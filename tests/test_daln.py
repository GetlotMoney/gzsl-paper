...
from model.candidates.v2.trainers.train_daln import load_config
...
def test_daln_config_and_boundary():
    config,_=load_config(ROOT/"config/tries/v2_try_072_daln_seed7.yaml"); assert config["idea_id"]=="IDEA-021"; source=(ROOT/"model/candidates/v2/trainers/train_daln.py").read_text(encoding="utf-8"); assert source.index("for epoch in range")<source.index("# official test严格在DALN训练结束后加载。")
