...
def test_full_attention_shapes_threshold_and_off_paths():
    ...
    expected_action_prob = torch.full((3, ACTION_COUNT), 1.0 / (ACTION_COUNT + 1))
    assert torch.allclose(state.utility, expected_action_prob)
    assert torch.allclose(state.max_utility, torch.full((3,), 1.0 / (ACTION_COUNT + 1)))
