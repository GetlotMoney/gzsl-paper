...
import torch
from model.frameworks.v4.ccgr import ClassConditionedGeometricGenerator,NeighborhoodResidualCCGR,tangent_direction_basis
from model.candidates.v2.trainers.train_ccgr import ccgr_class_features,harmonic_episode_loss,local_boundary_loss,load_config
...
def test_ccgr_config_and_boundary():
    config,_=load_config(ROOT/"config/tries/v2_try_058_ccgr_seed7.yaml"); assert config["idea_id"]=="IDEA-018"; source=(ROOT/"model/candidates/v2/trainers/train_ccgr.py").read_text(encoding="utf-8"); assert source.index("for epoch in range")<source.index("# official test严格在CCGR训练结束后加载。")
