...
WINDOW_SHA = "4e64cb1fa0a24b3fd734d53dc60dadf36ff65fb0e0a063bfdb74cb"
...
def _write_subset(
    ...
    *,
    ...
    manifest = {
        "schema_version": "gzsl-paper.svra-subset.v1",
        "subset": name,
...
def _make_config(
    ...
    *,
    ...
    text_sha = _write_json(
        text_manifest_path,
        {
            "schema_version": "mock.svra-text.v1",
            "outputs_sha256": {
    ...
    patch_manifest_sha = _write_json(
        patch_manifest_path,
        {
            "schema_version": "mock.svra-projected-patch.v1",
    ...
    bundle_dir = tmp_path / "action_bundle"
    ...
    bundle_sha = _write_json(
        bundle_path,
        {
            "schema_version": "gzsl-paper.svra-action-bundle.v1",
            "subsets": {
    ...
    return SVRAAssetConfig(
        ...
        action_bundle_manifest=ManifestContract(str(bundle_path), bundle_sha),
...
def test_subset_views_map_raw_global_indices_to_trainval_positions(tmp_path):
    ...
    assets, views = load_svra_gate_data(config, strict_sha=True, validate_tensor_values=False)
    try:
...
def test_dev_eval_crop_features_flag_or_output_is_rejected(tmp_path, flag, output_name):
    ...
    with pytest.raises(SVRADataError, match="crop_features_present|forbidden crop handles"):
        load_svra_gate_data(config, strict_sha=True, validate_tensor_values=False)
...
def test_train_eval_overlap_is_rejected(tmp_path):
    ...
    with pytest.raises(SVRADataError, match="overlap"):
        load_svra_gate_data(config, strict_sha=True, validate_tensor_values=False)
...
def test_train_eval_union_must_cover_trainval_axis(tmp_path):
    ...
    with pytest.raises(SVRADataError, match="union size"):
        load_svra_gate_data(config, strict_sha=True, validate_tensor_values=False)
