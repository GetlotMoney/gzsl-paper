...
def load_config(path):
    ...
    if config["schema_version"]!="gzsl-paper.vpa.v1" or config["attempt_id"] not in ("V2-TRY-118","V2-TRY-122","V2-TRY-123","V2-TRY-124","V2-TRY-128","V2-TRY-129") or config["idea_id"]!="IDEA-036": raise ValueError("VPA首次TRY身份错误。")
    expected_reverse={"V2-TRY-128":0.1,"V2-TRY-129":1.0}.get(config["attempt_id"],0.01)
    if int(config["epochs"])!=20 or int(config["batch_size"])!=256 or float(config["lr"])!=0.01 or float(config["weight_decay"])!=0.0 or float(config["forward_ridge"])!=0.01 or float(config["reverse_ridge"])!=expected_reverse or float(config["max_beta"])!=20.0: raise ValueError("VPA训练参数错误。")
...
def run(config_path,output_dir,expected_commit):
    ...
    try:
        ...
        model.load_state_dict(best_state,strict=True); torch.save({"attempt_id":config["attempt_id"],"code_commit":code_commit,"config":config,"selected_epoch":best_epoch,"vpa_state_dict":best_state,"history":history},output_dir/"vpa_model.pth"); parent_model=VisualPrototypeAttributeResidual(base_cra,visual_prototypes,config["max_beta"]).to(device); parent_metrics=evaluate(prototypes,ccgr.scale(),metric,parent_model,tensors,seenclasses,unseenclasses,device); candidate_metrics=evaluate(prototypes,ccgr.scale(),metric,model,tensors,seenclasses,unseenclasses,device); delta={key:candidate_metrics[key]-float(config["parent_metrics_percent"][key]) for key in ("U","S","H","ZS")}; beta=float(model.beta().detach()); threshold=79.5436092022288 if config["attempt_id"] in ("V2-TRY-128","V2-TRY-129") else 79.44820966336283; success=candidate_metrics["H"]>threshold and delta["U"]>=-2 and delta["S"]>=-2 and abs(beta)<19.6; atomic_write_json(output_dir/"data_fingerprints.json",{"files":input_sha}); metrics={"attempt_id":config["attempt_id"],"idea_id":config["idea_id"],"framework_id":config["framework_id"],"code_commit":code_commit,"config_sha256":config_sha,"base_config_sha256":base_config_sha,"evaluation_protocol":h1.EVALUATION_PROTOCOL,"test_used_for_selection":True,"unseen_images_used_for_gradient":False,"selected_epoch":best_epoch,"learned_beta":beta,"reverse_ridge":float(config["reverse_ridge"]),"recomputed_parent_metrics_percent":parent_metrics,"parent_metrics_percent":config["parent_metrics_percent"],"candidate_metrics_percent":candidate_metrics,"delta_vs_parent_percent_points":delta,"success":success,"vpa_model_sha256":sha256_file(output_dir/"vpa_model.pth")}; atomic_write_json(output_dir/"metrics.json",metrics); print(metrics); return metrics
    finally:
