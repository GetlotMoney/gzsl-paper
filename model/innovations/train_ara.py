...
def load_config(path):
    ...
    if not isinstance(config,dict) or actual!=CONFIG_KEYS: raise ValueError(f"ARA配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，多出={sorted(actual-CONFIG_KEYS)}。")
    if config["schema_version"]!="gzsl-paper.ara.v1" or config["attempt_id"] not in ("V2-TRY-092","V2-TRY-093","V2-TRY-094","V2-TRY-095") or config["idea_id"]!="IDEA-029": raise ValueError("ARA首次TRY身份错误。")
