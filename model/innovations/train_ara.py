...
def load_config(path):
    ...
    if config["schema_version"] not in ("gzsl-paper.ara.v1","gzsl-paper.ara.v2","gzsl-paper.ara.v3","gzsl-paper.ara.v4") or config["attempt_id"] not in ("V2-TRY-092","V2-TRY-093","V2-TRY-094","V2-TRY-095","V2-TRY-096","V2-TRY-097","V2-TRY-098","V2-TRY-099","V2-TRY-100","V2-TRY-104","V2-TRY-105","V2-TRY-106","V2-TRY-107","V2-TRY-109","V2-TRY-110","V2-TRY-111") or config["idea_id"] not in ("IDEA-029","IDEA-033"): raise ValueError("ARA首次TRY身份错误。")
    ...
    if config["attempt_id"] in ("V2-TRY-104","V2-TRY-105","V2-TRY-106","V2-TRY-107","V2-TRY-109","V2-TRY-110","V2-TRY-111") and (config["ridge_source"]!="class_centroids" or config["sdm_enabled"] is not False or config["idea_id"]!="IDEA-033"): raise ValueError("CRA类别中心ridge配置错误。")
...
def run(config_path,output_dir,expected_commit):
    ...
    try:
        ...
        last_state=copy.deepcopy(ara.state_dict()); ara.load_state_dict(best_state,strict=True); best_payload={"attempt_id":config["attempt_id"],"code_commit":code_commit,"config":config,"selected_epoch":best_epoch,"ara_state_dict":best_state,"history":history}; torch.save(best_payload,output_dir/"ara_model.pth"); shutil.copyfile(output_dir/"ara_model.pth",output_dir/"model_best.pth"); torch.save({"attempt_id":config["attempt_id"],"code_commit":code_commit,"config":config,"last_epoch":int(config["epochs"]),"ara_state_dict":last_state,"history":history},output_dir/"checkpoint_last.pth"); parent_ara=AttributeResidualAlignment(ridge_weight,attributes,config["max_beta"]).to(device); parent_metrics=evaluate(prototypes,ccgr.scale(),metric,parent_ara,tensors,seenclasses,unseenclasses,device); candidate_metrics=evaluate(prototypes,ccgr.scale(),metric,ara,tensors,seenclasses,unseenclasses,device); delta={key:candidate_metrics[key]-float(config["parent_metrics_percent"][key]) for key in ("U","S","H","ZS")}; beta=float(ara.beta().detach()); threshold=79.44820966336283 if config["attempt_id"] in ("V2-TRY-109","V2-TRY-110") else (79.38608193220331 if config["attempt_id"]=="V2-TRY-104" else 78.0); success=candidate_metrics["H"]>threshold and delta["U"]>=-2 and delta["S"]>=-2 and abs(beta)<19.6; atomic_write_json(output_dir/"data_fingerprints.json",{"files":input_sha}); metrics={"attempt_id":config["attempt_id"],"idea_id":config["idea_id"],"framework_id":config["framework_id"],"code_commit":code_commit,"config_sha256":config_sha,"base_config_sha256":base_config_sha,"evaluation_protocol":h1.EVALUATION_PROTOCOL,"test_used_for_selection":True,"unseen_images_used_for_gradient":False,"selected_epoch":best_epoch,"ridge":float(config["ridge"]),"ridge_source":config["ridge_source"],"learned_beta":beta,"sdm_enabled":bool(config["sdm_enabled"]),"ccgr_enabled":bool(config["ccgr_enabled"]),"recomputed_parent_metrics_percent":parent_metrics,"parent_metrics_percent":config["parent_metrics_percent"],"candidate_metrics_percent":candidate_metrics,"delta_vs_parent_percent_points":delta,"success":success,"ara_model_sha256":sha256_file(output_dir/"ara_model.pth"),"model_best_sha256":sha256_file(output_dir/"model_best.pth"),"checkpoint_last_sha256":sha256_file(output_dir/"checkpoint_last.pth")}; atomic_write_json(output_dir/"metrics.json",metrics); print(metrics); return metrics
    finally:
