...
def load_config(path):
    ...
    expected=CONFIG_KEYS_V2 if isinstance(config,dict) and config.get("schema_version")=="gzsl-paper.ara.v2" else CONFIG_KEYS
    ...
    if config["schema_version"] not in ("gzsl-paper.ara.v1","gzsl-paper.ara.v2") or config["attempt_id"] not in ("V2-TRY-092","V2-TRY-093","V2-TRY-094","V2-TRY-095","V2-TRY-096") or config["idea_id"]!="IDEA-029": raise ValueError("ARA首次TRY身份错误。")
    ...
    if config["attempt_id"]=="V2-TRY-096" and config["sdm_enabled"] is not False: raise ValueError("ARA消融必须关闭SDM。")
