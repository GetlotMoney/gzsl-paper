...
from model.innovations.cara import ConfidenceAwareAttributeResidual
...
CONFIG_KEYS={"schema_version","attempt_id","idea_id","framework_id","base_config","ccgr_model","ccgr_model_sha256","ara_model","ara_model_sha256","seed","epochs","batch_size","lr","weight_decay","ridge","max_beta","max_beta_residual","parent_metrics_percent"}
...
def load_config(path):
    ...
    if not isinstance(config,dict) or actual!=CONFIG_KEYS: raise ValueError(f"CARA配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，多出={sorted(actual-CONFIG_KEYS)}。")
    if config["schema_version"]!="gzsl-paper.cara.v1" or config["attempt_id"]!="V2-TRY-102" or config["idea_id"]!="IDEA-031": raise ValueError("CARA首次TRY身份错误。")
    if int(config["epochs"])!=20 or int(config["batch_size"])!=256 or float(config["lr"])!=0.001 or float(config["weight_decay"])!=0.0001 or float(config["ridge"])!=0.01 or float(config["max_beta"])!=20.0 or float(config["max_beta_residual"])!=4.0: raise ValueError("CARA训练参数错误。")
...
def run(config_path,output_dir,expected_commit):
    ...
    base_config,base_config_sha=h1.load_config(base_path); paths=h1.resolve_paths(base_config); input_sha=h1.verify_inputs(base_config,paths,h1.TRAINING_KEYS); ccgr_path=Path(config["ccgr_model"]); ara_path=Path(config["ara_model"])
    if sha256_file(ccgr_path)!=config["ccgr_model_sha256"] or sha256_file(ara_path)!=config["ara_model_sha256"]: raise ValueError("CARA父模型SHA不匹配。")
    attribute_sha=sha256_file(paths["att_splits"])
    if attribute_sha!=base_config["expected_sha256"]["att_splits"]: raise ValueError("CARA属性文件SHA不匹配。")
    input_sha["att_splits"]=attribute_sha; device=torch.device(base_config["device"])
    if device.type!="cuda" or not torch.cuda.is_available(): raise RuntimeError("CARA要求CUDA。")
    ...
    try:
        ...
        for epoch in range(1,int(config["epochs"])+1):
            ...
            official=evaluate(prototypes,ccgr.scale(),model,tensors,seenclasses,unseenclasses,device); stats=model.residual_stats(tensors["seen_features"].to(device)); row={"epoch":epoch,"loss":loss_sum/count,"official_metrics_percent":official,"residual_stats":stats}; history.append(row); print(f"epoch={epoch} loss={row['loss']:.6f} official_H={official['H']:.6f} residual_std={stats['std']:.6f}")
            if official["H"]>best_H: best_H=official["H"]; best_epoch=epoch; best_state=copy.deepcopy(model.state_dict())
        model.load_state_dict(best_state,strict=True); torch.save({"attempt_id":config["attempt_id"],"code_commit":code_commit,"config":config,"selected_epoch":best_epoch,"cara_state_dict":best_state,"history":history},output_dir/"cara_model.pth"); parent_model=ConfidenceAwareAttributeResidual(base_ara,config["max_beta_residual"]).to(device); parent_metrics=evaluate(prototypes,ccgr.scale(),parent_model,tensors,seenclasses,unseenclasses,device); candidate_metrics=evaluate(prototypes,ccgr.scale(),model,tensors,seenclasses,unseenclasses,device); delta={key:candidate_metrics[key]-float(config["parent_metrics_percent"][key]) for key in ("U","S","H","ZS")}; stats=model.residual_stats(torch.cat((tensors["seen_features"],tensors["unseen_features"])).to(device)); success=candidate_metrics["H"]>79.38608193220331 and delta["U"]>=-2 and delta["S"]>=-2 and stats["std"]>0.01; atomic_write_json(output_dir/"data_fingerprints.json",{"files":input_sha}); metrics={"attempt_id":config["attempt_id"],"idea_id":config["idea_id"],"framework_id":config["framework_id"],"code_commit":code_commit,"config_sha256":config_sha,"base_config_sha256":base_config_sha,"evaluation_protocol":h1.EVALUATION_PROTOCOL,"test_used_for_selection":True,"unseen_images_used_for_gradient":False,"selected_epoch":best_epoch,"residual_stats":stats,"recomputed_parent_metrics_percent":parent_metrics,"parent_metrics_percent":config["parent_metrics_percent"],"candidate_metrics_percent":candidate_metrics,"delta_vs_parent_percent_points":delta,"success":success,"cara_model_sha256":sha256_file(output_dir/"cara_model.pth")}; atomic_write_json(output_dir/"metrics.json",metrics); print(metrics); return metrics
    finally:
