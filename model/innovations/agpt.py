...
class AmbiguityGatedPatchTieBreaker(nn.Module):
    """仅用局部patch证据修正同族低margin top2。"""
    ...
    def __init__(
        ...
        if tuple(sdcr_prototypes.shape) != (200, 768):
            raise ValueError("AGPT SDCR原型必须是[200,768]。")
        if tuple(group_ids.shape) != (200,):
            raise ValueError("AGPT group_ids必须是[200]。")
        if float(margin_threshold) <= 0 or float(margin_temperature) <= 0:
            raise ValueError("AGPT margin阈值和温度必须为正。")
    ...
    def gate_values(
        ...
        ) * same_group.to(logits.dtype)
        return gate, same_group, top.indices

    def stats(self) -> dict[str, float]:
        return {
            "patch_beta": float(self.beta().detach()),
    ...
    def forward(
        ...
        logits = parent_logits + self.sdcr_beta * (
            F.normalize(images.float(), dim=-1) @ sdcr.T
        )
        if not enabled:
            return logits
        local_scores = patch_scores.to(logits.device).float()
        if local_scores.shape[1] == 200 and ids.numel() != 200:
            local_scores = local_scores.index_select(1, ids)
        if local_scores.shape != logits.shape:
            raise ValueError("AGPT patch分数与候选logits形状不一致。")
        gate, _, top_positions = self.gate_values(logits, ids)
        top_scores = local_scores.gather(1, top_positions)
