...
CONFIG_KEYS = {
    ...
    "claude_embeddings", "claude_embeddings_sha256", "max_parent_projection",
...
def load_config(path: Path):
    ...
    if not isinstance(config, dict) or actual != CONFIG_KEYS:
        raise ValueError(
            f"PBOR配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，"
            f"多出={sorted(actual-CONFIG_KEYS)}。"
        )
    if (
        config["schema_version"] != "gzsl-paper.pbor.v1"
        or config["experiment_id"] != "V2-INNOVATION-033"
        or config["idea_id"] != "IDEA-067"
    ):
        raise ValueError("PBOR身份错误。")
    if (
        ...
        raise ValueError("PBOR协议边界错误。")
    if config["text_cache_provenance_complete"] is not False:
        raise ValueError("PBOR文本cache provenance未完整。")
    if (
        ...
        raise ValueError("PBOR Chen训练量错误。")
    if (
        config["optimizer"] != "Adam"
        or float(config["learning_rate"]) != 0.01
        or float(config["weight_decay"]) != 0.0
        or float(config["max_parent_projection"]) != 1.0
    ):
        raise ValueError("PBOR优化参数错误。")
...
def run(config_path: Path, output_dir: Path, expected_commit: str, run_id: str):
    ...
    for key in (
        ...
        if sha256_file(Path(config[key])) != config[f"{key}_sha256"]:
            raise ValueError(f"PBOR {key} SHA错误。")
    ...
    try:
        ...
        if not torch.equal(checked_seen, seen_classes) or not torch.equal(checked_unseen, unseen_classes):
            raise ValueError("PBOR CUB类别边界错误。")
        ...
        model = PartialBiOrthogonalResidual(
            claude, class_names, parent.prototypes().detach(), fixed_beta,
            float(config["max_parent_projection"]),
        ).to(device)
        ...
        for key in ("U", "S", "H", "ZS"):
            if abs(best_metrics[key] - float(expected_parent[key])) > 1e-5:
                raise ValueError(f"PBOR初始态未复现OCLR：{key}。")
        ...
        for iteration in range(int(config["niters"])):
            ...
            if iteration % int(config["report_interval"]) == 0:
                ...
                projection = float(model.parent_projection().detach())
                history.append(
                    {
                        "iteration": iteration,
                ...
                print(
                    ...
                    f"best_H={best_h:.6f} projection={projection:.6f}"
                )

        atomic_torch_save(
            output_dir / "checkpoint_last.pth",
            {
                "pbor_state_dict": copy.deepcopy(model.state_dict()),
        ...
        best_projection = float(
            torch.tanh(best_state["raw_parent_projection"])
            * float(config["max_parent_projection"])
        )
        metrics = {
            ...
            "selected_iteration": best_iteration,
            "fixed_beta": fixed_beta,
            "learned_parent_projection": best_projection,
