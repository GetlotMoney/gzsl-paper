...
from model.innovations.ccpe import ClassConditionedPatchEvidence
...
CONFIG_KEYS = {
    ...
    "patch_chunk_size", "max_beta", "device", "random_seed", "batch_size",
...
def load_config(path: Path):
    ...
    if not isinstance(config, dict) or actual != CONFIG_KEYS:
        raise ValueError(
            f"SPCR配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，"
            f"多出={sorted(actual-CONFIG_KEYS)}。"
        )
    if (
        config["schema_version"] != "gzsl-paper.spcr.v1"
        or config["experiment_id"] != "V2-INNOVATION-052"
        or config["idea_id"] != "IDEA-086"
    ):
        raise ValueError("SPCR身份错误。")
    if (
        ...
        raise ValueError("SPCR协议边界错误。")
    if config["feature_provenance_complete"] is not False:
        raise ValueError("SPCR patch provenance未完整。")
    if set(config["patch_inputs"]) != {"train", "seen", "unseen"} or set(
        config["patch_sha256"]
    ) != {"train", "seen", "unseen"}:
        raise ValueError("SPCR patch输入/SHA必须包含train/seen/unseen。")
    if (
        ...
        raise ValueError("SPCR patch或训练参数错误。")
...
def compose_logits(
    ...
    logits = sdcr(parent_logits, images, class_ids)
    return patch_model(logits, patch_scores, class_ids)
...
def evaluate(
    ...
    def predict(features, patch_scores, class_ids=None):
        ...
        predictions = compose_logits(
            logits, sdcr, patch_model, images, local_scores, ids
        ).argmax(1).cpu()
...
def run(config_path: Path, output_dir: Path, expected_commit: str, run_id: str):
    ...
    for key in (
        ...
        if sha256_file(Path(config[key])) != config[f"{key}_sha256"]:
            raise ValueError(f"SPCR {key} SHA错误。")
    ...
    try:
        ...
        if not torch.equal(checked_seen, seen_classes) or not torch.equal(
            ...
            raise ValueError("SPCR CUB类别边界错误。")
        ...
        print("precomputing SDCR+CCPE top2 patch scores")
        scores = _precompute_scores(config, text_residual, device)
        if scores["train"].shape != (labels.shape[0], 200):
            raise ValueError("SPCR train patch score形状错误。")
        if scores["seen"].shape != (official["seen_labels"].shape[0], 200):
            raise ValueError("SPCR seen patch score形状错误。")
        if scores["unseen"].shape != (official["unseen_labels"].shape[0], 200):
            raise ValueError("SPCR unseen patch score形状错误。")

        patch_model = ClassConditionedPatchEvidence(
            max_beta=float(config["max_beta"])
        ).to(device)
        optimizer = torch.optim.Adam(
            patch_model.parameters(), lr=float(config["learning_rate"]), weight_decay=0.0
        )
        ...
        best_metrics = evaluate(
            parent, sdrs, calibrator, sdcr, patch_model, official, scores,
        ...
        for key in ("U", "S", "H", "ZS"):
            if abs(best_metrics[key] - float(expected_parent[key])) > 1e-5:
                raise ValueError(f"SPCR初始态未复现SDCR：{key}。")
        ...
        for iteration in range(int(config["niters"])):
            ...
            logits = F.normalize(images, dim=-1) @ parent_prototypes.index_select(0, class_ids).T * scale
            logits = sdrs(logits, images, class_ids)
            logits = calibrator(logits, seen_mask)
            logits = compose_logits(
                logits, sdcr, patch_model, images, patch_scores, class_ids
            )
            loss = F.cross_entropy(logits, targets)
            optimizer.zero_grad(set_to_none=True)
            loss.backward()
            require_finite_gradients(patch_model)
            optimizer.step()
            ...
            if iteration % int(config["report_interval"]) == 0:
                ...
                beta = float(patch_model.beta().detach())
                history.append(
                    {
                        "iteration": iteration,
                ...
                print(
                    ...
                    f"best_H={best_h:.6f} patch_beta={beta:.6f}"
                )

        atomic_torch_save(
            output_dir / "checkpoint_last.pth",
            {
                "patch_state_dict": copy.deepcopy(patch_model.state_dict()),
        ...
        patch_model.load_state_dict(best_state, strict=True)
        best_beta = float(patch_model.beta().detach())
        atomic_write_json(
