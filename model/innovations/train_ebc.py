...
def load_config(path):
    ...
    if config["schema_version"]!="gzsl-paper.ebc.v1" or config["attempt_id"]!="V2-TRY-113" or config["idea_id"]!="IDEA-035": raise ValueError("EBC首次TRY身份错误。")
    if int(config["epochs"])!=20 or int(config["batch_half"])!=32 or float(config["lr"])!=0.01 or float(config["weight_decay"])!=0.0 or float(config["ridge"])!=0.01 or float(config["max_beta"])!=20.0 or float(config["max_gamma"])!=0.2: raise ValueError("EBC训练参数错误。")
