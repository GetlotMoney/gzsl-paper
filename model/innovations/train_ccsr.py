...
from model.innovations.ccsr import ClassConditionedSentenceRouting
...
CONFIG_KEYS = {
    ...
    "eight_sentence_embeddings", "eight_sentence_embeddings_sha256", "max_delta",
...
def load_config(path: Path):
    ...
    if not isinstance(config, dict) or actual != CONFIG_KEYS:
        raise ValueError(
            f"CCSR配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，"
            f"多出={sorted(actual-CONFIG_KEYS)}。"
        )
    if (
        config["schema_version"] != "gzsl-paper.ccsr.v1"
        or config["experiment_id"] != "V2-INNOVATION-039"
        or config["idea_id"] != "IDEA-073"
    ):
        raise ValueError("CCSR身份错误。")
    if (
        ...
        raise ValueError("CCSR协议边界错误。")
    if config["text_cache_provenance_complete"] is not False:
        raise ValueError("CCSR文本cache provenance未完整。")
    if (
        ...
        or float(config["max_delta"]) != 2.0
    ):
        raise ValueError("CCSR训练参数错误。")
...
def run(config_path: Path, output_dir: Path, expected_commit: str, run_id: str):
    ...
    for key in (
        ...
        if sha256_file(Path(config[key])) != config[f"{key}_sha256"]:
            raise ValueError(f"CCSR {key} SHA错误。")
    ...
    try:
        ...
        if not torch.equal(checked_seen, seen_classes) or not torch.equal(checked_unseen, unseen_classes):
            raise ValueError("CCSR CUB类别边界错误。")
        ...
        model = ClassConditionedSentenceRouting(
            new_sentences, class_names, seen_classes.to(device), base_weights,
            fixed_beta, float(config["max_delta"]),
        ).to(device)
        ...
        for key in ("U", "S", "H", "ZS"):
            if abs(best_metrics[key] - float(expected_parent[key])) > 1e-5:
                raise ValueError(f"CCSR初始态未复现CASR：{key}。")
        ...
        for iteration in range(int(config["niters"])):
            ...
            if iteration % int(config["report_interval"]) == 0:
                ...
                print(
                    ...
                    f"best_H={best_h:.6f} delta={stats['delta']:.6f} "
        ...
        atomic_torch_save(
            ...
            {
                "ccsr_state_dict": copy.deepcopy(model.state_dict()),
