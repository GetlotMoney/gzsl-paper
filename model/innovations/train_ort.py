...
from model.innovations.train_elpt import FrozenPrototypeClassifier, _fold_package, _load_fold_checkpoint
...
def load_config(path:Path):
    ...
    if not isinstance(config,dict) or actual!=CONFIG_KEYS: raise ValueError(f"ORT配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，多出={sorted(actual-CONFIG_KEYS)}。")
    if config["schema_version"]!="gzsl-paper.ort.v1" or config["attempt_id"]!="V2-TRY-056" or config["idea_id"]!="IDEA-017": raise ValueError("ORT首次TRY身份错误。")
