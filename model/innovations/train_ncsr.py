...
CONFIG_KEYS = {
    ...
    "eight_sentence_embeddings_sha256", "neighbor_k", "max_gamma", "device",
...
def load_config(path: Path):
    ...
    if not isinstance(config, dict) or actual != CONFIG_KEYS:
        raise ValueError(
            f"NCSR配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，"
            f"多出={sorted(actual-CONFIG_KEYS)}。"
        )
    if (
        config["schema_version"] != "gzsl-paper.ncsr.v1"
        or config["experiment_id"] != "V2-INNOVATION-047"
        or config["idea_id"] != "IDEA-081"
    ):
        raise ValueError("NCSR身份错误。")
    if (
        ...
        raise ValueError("NCSR协议边界错误。")
    if config["text_cache_provenance_complete"] is not False:
        raise ValueError("NCSR文本cache provenance未完整。")
    if (
        int(config["neighbor_k"]) != 5
        or float(config["max_gamma"]) != 5.0
...
def run(config_path: Path, output_dir: Path, expected_commit: str, run_id: str):
    ...
    for key in (
        ...
        if sha256_file(Path(config[key])) != config[f"{key}_sha256"]:
            raise ValueError(f"NCSR {key} SHA错误。")
    ...
    try:
        ...
        if not torch.equal(checked_seen, seen_classes) or not torch.equal(
            ...
            raise ValueError("NCSR CUB类别边界错误。")
        ...
        for parameter in sdcr.parameters():
            parameter.requires_grad_(False)
        base_residual_prototypes = sdcr.prototypes(use_dropout=False).detach()
        model = NeighborhoodContrastiveSemanticResidual(
            base_residual_prototypes,
            fixed_beta,
            int(config["neighbor_k"]),
            float(config["max_gamma"]),
        ).to(device)
        ...
        for key in ("U", "S", "H", "ZS"):
            if abs(best_metrics[key] - float(expected_parent[key])) > 1e-5:
                raise ValueError(f"NCSR初始态未复现SDCR：{key}。")
        ...
        for iteration in range(int(config["niters"])):
            ...
            if iteration % int(config["report_interval"]) == 0:
                ...
                print(
                    ...
                    f"best_H={best_h:.6f} gamma={stats['gamma']:.6f}"
                )

        atomic_torch_save(
            output_dir / "checkpoint_last.pth",
            {
                "ncsr_state_dict": copy.deepcopy(model.state_dict()),
