...
def load_config(path:Path):
    ...
    if not isinstance(config,dict) or actual!=CONFIG_KEYS: raise ValueError(f"SVPG配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，多出={sorted(actual-CONFIG_KEYS)}。")
    if config["schema_version"]!="gzsl-paper.svpg.v1" or config["attempt_id"]!="V2-TRY-053" or config["idea_id"]!="IDEA-016": raise ValueError("SVPG首次TRY身份错误。")
    if int(config["epochs"])!=20 or int(config["batch_size"])!=64 or float(config["lr"])!=0.001 or float(config["weight_decay"])!=0.0001: raise ValueError("SVPG训练参数错误。")
