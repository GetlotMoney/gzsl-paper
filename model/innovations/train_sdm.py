...
def load_config(path:Path):
    ...
    expected=CONFIG_KEYS_LOW_RANK if isinstance(config,dict) and config.get("schema_version")=="gzsl-paper.sdm-low-rank.v1" else CONFIG_KEYS
    ...
    if config["schema_version"] not in ("gzsl-paper.sdm.v1","gzsl-paper.sdm-low-rank.v1") or config["attempt_id"] not in ("V2-TRY-086","V2-TRY-087") or config["idea_id"]!="IDEA-028": raise ValueError("SDM首次TRY身份错误。")
