...
def load_config(path:Path):
    ...
    if not isinstance(config,dict) or actual!=CONFIG_KEYS: raise ValueError(f"SDM配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，多出={sorted(actual-CONFIG_KEYS)}。")
    if config["schema_version"]!="gzsl-paper.sdm.v1" or config["attempt_id"]!="V2-TRY-086" or config["idea_id"]!="IDEA-028": raise ValueError("SDM首次TRY身份错误。")
