...
CONFIG_KEYS = {
    ...
    "class_name_embeddings_sha256", "patch_inputs", "patch_sha256", "pool_top_k",
    "pool_chunk_size", "device", "random_seed", "batch_size", "epochs", "niters",
...
def load_config(path: Path):
    ...
    if not isinstance(config, dict) or actual != CONFIG_KEYS:
        raise ValueError(
            f"LPSR配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，"
            f"多出={sorted(actual-CONFIG_KEYS)}。"
        )
    if (
        config["schema_version"] != "gzsl-paper.lpsr.v1"
        or config["experiment_id"] != "V2-INNOVATION-014"
        or config["idea_id"] != "IDEA-048"
    ):
        raise ValueError("LPSR身份错误。")
    if (
    ...
    if set(config["patch_inputs"]) != {"train", "seen", "unseen"}:
        raise ValueError("LPSR patch输入必须包含train/seen/unseen。")
    if set(config["patch_sha256"]) != {"train", "seen", "unseen"}:
        raise ValueError("LPSR patch SHA必须包含train/seen/unseen。")
    if (
        int(config["pool_top_k"]) != 64
        or int(config["pool_chunk_size"]) != 32
        or int(config["batch_size"]) != 50
        or int(config["epochs"]) != 200
        or int(config["niters"]) != 28228
        or int(config["report_interval"]) != 141
    ):
        raise ValueError("LPSR pooling或Chen训练量错误。")
    if (
        ...
        raise ValueError("LPSR优化参数错误。")
    return config, sha256_file(path)


def _load_patch_features(config, device):
    pooled = {}
    for split, path_text in config["patch_inputs"].items():
        path = h1.repo_path(path_text)
        if sha256_file(path) != config["patch_sha256"][split]:
            raise ValueError(f"LPSR {split} patch SHA错误。")
        patches = torch.load(path, map_location="cpu", weights_only=True)
        pooled[split] = pool_fgvd_local_features(
            patches, int(config["pool_top_k"]), device,
            chunk_size=int(config["pool_chunk_size"]),
        )
        del patches
    return pooled
...
@torch.no_grad()
def evaluate(
    parent, sdrs, calibrator, model, tensors, local, seen_classes, unseen_classes, device
):
    prototypes = parent.prototypes()

    def predict(features, local_features, class_ids=None):
        ids = torch.arange(200, device=device) if class_ids is None else class_ids.to(device)
        images = features.to(device).float()
        local_images = local_features.to(device).float()
        ...
        predictions = model(logits, local_images, ids).argmax(1).cpu()
        return predictions if class_ids is None else class_ids[predictions]

    seen_predictions = predict(
        tensors["seen_features"], local["seen"]
    )
    unseen_predictions = predict(
        tensors["unseen_features"], local["unseen"]
    )
    zsl_predictions = predict(
        tensors["unseen_features"], local["unseen"], unseen_classes
    )
...
def run(config_path: Path, output_dir: Path, expected_commit: str, run_id: str):
    ...
    for key in ("base_model", "sdrs_model", "sebc_model", "class_name_embeddings"):
        ...
        if sha256_file(path) != config[f"{key}_sha256"]:
            raise ValueError(f"LPSR {key} SHA错误。")
    ...
    try:
        ...
        if not torch.equal(checked_seen, seen_classes) or not torch.equal(checked_unseen, unseen_classes):
            raise ValueError("LPSR CUB类别边界错误。")
        ...
        print("pooling frozen patch features")
        local = _load_patch_features(config, device)
        if local["train"].shape[0] != labels.shape[0]:
            raise ValueError("LPSR train patch数量与标签不一致。")
        if local["seen"].shape[0] != official["seen_labels"].shape[0]:
            raise ValueError("LPSR test-seen patch数量与标签不一致。")
        if local["unseen"].shape[0] != official["unseen_labels"].shape[0]:
            raise ValueError("LPSR test-unseen patch数量与标签不一致。")

        model = LocalPatchSemanticResidual(
            sentence, names, max_beta=float(config["max_beta"])
        ).to(device)
        ...
        best_metrics = evaluate(
            parent, sdrs, calibrator, model, official, local,
        ...
        for key in ("U", "S", "H", "ZS"):
            if abs(best_metrics[key] - float(expected_parent[key])) > 1e-5:
                raise ValueError(f"LPSR关闭态未复现SEBC父指标：{key}。")
        ...
        for iteration in range(int(config["niters"])):
            ...
            local_images = local["train"].index_select(0, indices).to(device).float()
            ...
            logits = model(parent_logits, local_images, class_ids)
        ...
        atomic_torch_save(
            ...
            {
                "lpsr_state_dict": copy.deepcopy(model.state_dict()),
