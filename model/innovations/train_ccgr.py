...
def load_config(path:Path):
    ...
    expected=CONFIG_KEYS_V4 if schema=="gzsl-paper.ccgr.v4" else (CONFIG_KEYS_V3 if schema in ("gzsl-paper.ccgr.v3","gzsl-paper.ccgr.tune.v1") else (CONFIG_KEYS_V2 if schema=="gzsl-paper.ccgr.v2" else CONFIG_KEYS))
    ...
    if config["schema_version"] not in ("gzsl-paper.ccgr.v1","gzsl-paper.ccgr.v2","gzsl-paper.ccgr.v3","gzsl-paper.ccgr.v4","gzsl-paper.ccgr.tune.v1") or config["attempt_id"] not in ("V2-TRY-058","V2-TRY-059","V2-TRY-060","V2-TRY-061","V2-TRY-062","V2-TRY-063","V2-TRY-064","V2-TRY-065","V2-TRY-066","V2-TRY-067") or config["idea_id"]!="IDEA-018": raise ValueError("CCGR首次TRY身份错误。")
    ...
    expected_max={"V2-TRY-066":0.15,"V2-TRY-067":0.2}.get(config["attempt_id"],0.1)
    ...
    if config["attempt_id"] in ("V2-TRY-060","V2-TRY-062","V2-TRY-063","V2-TRY-064","V2-TRY-065","V2-TRY-066","V2-TRY-067") and (not config["fold_checkpoint_dir"] or int(config["batch_half"])!=32 or float(config["pseudo_unseen_weight"])!=0.25): raise ValueError("CCGR unseen风险配置错误。")
