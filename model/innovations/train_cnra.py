...
CONFIG_KEYS={"schema_version","attempt_id","idea_id","framework_id","base_config","ccgr_model","ccgr_model_sha256","vpa_model","vpa_model_sha256","jbec_model","jbec_model_sha256","class_name_embeddings","class_name_embeddings_sha256","seed","epochs","batch_size","lr","weight_decay","max_beta","parent_metrics_percent"}
...
def parent_logits(images,prototypes,scale,cra,visual,beta_visual,gamma,seen_mask,class_ids=None):
    ids=torch.arange(prototypes.shape[0],device=images.device) if class_ids is None else class_ids.to(images.device); x=F.normalize(images.float(),dim=-1); selected_p=prototypes.index_select(0,ids); selected_v=visual.index_select(0,ids); attr=F.normalize(x@cra.ridge_weight,dim=-1)@cra.class_attributes.index_select(0,ids).T; selected_seen=seen_mask.index_select(0,ids); return x@selected_p.T*scale+cra.beta()*attr+beta_visual*(x@selected_v.T)-gamma*selected_seen.to(x.dtype).unsqueeze(0)
