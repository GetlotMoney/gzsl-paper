...
from model.innovations.sfa import SemanticFactorAlignment,fit_ridge_factor_map,semantic_factor_matrix
...
CONFIG_KEYS={"schema_version","attempt_id","idea_id","framework_id","base_config","ccgr_model","ccgr_model_sha256","seed","epochs","batch_size","lr","weight_decay","ridge","factor_rank","max_beta","parent_metrics_percent"}
...
def load_config(path):
    ...
    if not isinstance(config,dict) or actual!=CONFIG_KEYS: raise ValueError(f"SFA配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，多出={sorted(actual-CONFIG_KEYS)}。")
    if config["schema_version"]!="gzsl-paper.sfa.v1" or config["attempt_id"]!="V2-TRY-103" or config["idea_id"]!="IDEA-032": raise ValueError("SFA首次TRY身份错误。")
    if int(config["epochs"])!=20 or int(config["batch_size"])!=256 or float(config["lr"])!=0.01 or float(config["weight_decay"])!=0.0 or float(config["ridge"])!=0.01 or int(config["factor_rank"])!=64 or float(config["max_beta"])!=20.0: raise ValueError("SFA训练参数错误。")
...
def run(config_path,output_dir,expected_commit):
    ...
    if sha256_file(ccgr_path)!=config["ccgr_model_sha256"]: raise ValueError("SFA父CCGR模型SHA不匹配。")
    device=torch.device(base_config["device"])
    if device.type!="cuda" or not torch.cuda.is_available(): raise RuntimeError("SFA要求CUDA。")
    ...
    try:
        ...
        for epoch in range(1,int(config["epochs"])+1):
            ...
            for start in range(0,labels.numel(),int(config["batch_size"])):
                indices=order[start:start+int(config["batch_size"])]; targets=mapping[labels[indices]].to(device); logits=parent_train.index_select(0,indices.to(device))+model.beta()*residual_train.index_select(0,indices.to(device)); loss=F.cross_entropy(logits,targets); optimizer.zero_grad(set_to_none=True); loss.backward(); optimizer.step(); loss_sum+=float(loss.detach()); count+=1
            official=evaluate(prototypes,ccgr.scale(),model,tensors,seenclasses,unseenclasses,device); row={"epoch":epoch,"loss":loss_sum/count,"official_metrics_percent":official,"beta":float(model.beta().detach())}; history.append(row); print(f"epoch={epoch} loss={row['loss']:.6f} official_H={official['H']:.6f} beta={row['beta']:.6f}")
            if official["H"]>best_H: best_H=official["H"]; best_epoch=epoch; best_state=copy.deepcopy(model.state_dict())
        model.load_state_dict(best_state,strict=True); torch.save({"attempt_id":config["attempt_id"],"code_commit":code_commit,"config":config,"selected_epoch":best_epoch,"sfa_state_dict":best_state,"history":history},output_dir/"sfa_model.pth"); parent_model=SemanticFactorAlignment(ridge_weight,factors,config["max_beta"]).to(device); parent_metrics=evaluate(prototypes,ccgr.scale(),parent_model,tensors,seenclasses,unseenclasses,device); candidate_metrics=evaluate(prototypes,ccgr.scale(),model,tensors,seenclasses,unseenclasses,device); delta={key:candidate_metrics[key]-float(config["parent_metrics_percent"][key]) for key in ("U","S","H","ZS")}; beta=float(model.beta().detach()); success=candidate_metrics["H"]>=78.0 and delta["U"]>=-2 and delta["S"]>=-2 and abs(beta)<19.6; atomic_write_json(output_dir/"data_fingerprints.json",{"files":input_sha}); metrics={"attempt_id":config["attempt_id"],"idea_id":config["idea_id"],"framework_id":config["framework_id"],"code_commit":code_commit,"config_sha256":config_sha,"base_config_sha256":base_config_sha,"evaluation_protocol":h1.EVALUATION_PROTOCOL,"test_used_for_selection":True,"unseen_images_used_for_gradient":False,"selected_epoch":best_epoch,"factor_rank":int(config["factor_rank"]),"learned_beta":beta,"recomputed_parent_metrics_percent":parent_metrics,"parent_metrics_percent":config["parent_metrics_percent"],"candidate_metrics_percent":candidate_metrics,"delta_vs_parent_percent_points":delta,"success":success,"sfa_model_sha256":sha256_file(output_dir/"sfa_model.pth")}; atomic_write_json(output_dir/"metrics.json",metrics); print(metrics); return metrics
    finally:
