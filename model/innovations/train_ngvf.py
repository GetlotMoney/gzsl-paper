...
from model.innovations.ngvf import NormalizedGeometricVisualFusion
...
def load_config(path):
    ...
    if not isinstance(config,dict) or actual!=CONFIG_KEYS: raise ValueError(f"NGVF配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，多出={sorted(actual-CONFIG_KEYS)}。")
    if config["schema_version"]!="gzsl-paper.ngvf.v1" or config["attempt_id"]!="V2-TRY-137" or config["idea_id"]!="IDEA-040": raise ValueError("NGVF首次TRY身份错误。")
    if int(config["epochs"])!=20 or int(config["batch_half"])!=32 or float(config["lr"])!=0.01 or float(config["weight_decay"])!=0.0 or float(config["forward_ridge"])!=0.01 or float(config["reverse_ridge"])!=0.01 or float(config["max_beta"])!=20.0 or float(config["max_vpa_beta"])!=20.0 or float(config["max_gamma"])!=0.3: raise ValueError("NGVF训练参数错误。")
    return config,sha256_file(path)


def _load_gate(config,device):
    path=Path(config["ntr_gate_model"])
    if sha256_file(path)!=config["ntr_gate_model_sha256"]: raise ValueError("NGVF父NTR gate SHA不匹配。")
...
def paired_logits(images,prototypes,scale,cra,visual_prototypes,beta_visual,gamma,seen_mask,identity,class_ids=None):
    ids=torch.arange(prototypes.shape[0],device=images.device) if class_ids is None else class_ids.to(images.device); selected_p=prototypes.index_select(0,ids); selected_v=visual_prototypes.index_select(0,ids); x=F.normalize(images.float(),dim=-1); cra_logits=cra.logits(images,prototypes,scale,identity,class_ids); main=x@selected_p.T*scale; attribute=cra_logits-main; visual=x@selected_v.T; selected_seen=seen_mask.index_select(0,ids); additive=cra_logits+beta_visual*visual-gamma*selected_seen.to(x.dtype).unsqueeze(0); fused=F.normalize(selected_p+(beta_visual/scale)*selected_v,dim=-1); normalized=x@fused.T*scale+attribute-gamma*selected_seen.to(x.dtype).unsqueeze(0); return additive,normalized


@torch.no_grad()
def evaluate(prototypes,scale,cra,visual,beta,gamma,fusion,tensors,seenclasses,unseenclasses,device):
    ...
    def predict(features,class_ids=None):
        additive,normalized=paired_logits(features.to(device).float(),prototypes,scale,cra,visual,beta,gamma,seen_mask,identity,class_ids); result=fusion(additive,normalized).argmax(1).cpu(); return result if class_ids is None else class_ids[result]
...
def run(config_path,output_dir,expected_commit):
    ...
    base_config,base_config_sha=h1.load_config(base_path); paths=h1.resolve_paths(base_config); input_sha=h1.verify_inputs(base_config,paths,h1.TRAINING_KEYS); specs=((Path(config["base_checkpoint"]),config["base_checkpoint_sha256"]),(Path(config["ccgr_model"]),config["ccgr_model_sha256"]),(Path(config["cra_model"]),config["cra_model_sha256"]),(Path(config["vpa_model"]),config["vpa_model_sha256"]),(Path(config["jbec_model"]),config["jbec_model_sha256"]))
    if any(sha256_file(path)!=expected for path,expected in specs): raise ValueError("NGVF父模型SHA不匹配。")
    attribute_sha=sha256_file(paths["att_splits"])
    if attribute_sha!=base_config["expected_sha256"]["att_splits"]: raise ValueError("NGVF属性文件SHA不匹配。")
    input_sha["att_splits"]=attribute_sha; device=torch.device(base_config["device"])
    if device.type!="cuda" or not torch.cuda.is_available(): raise RuntimeError("NGVF要求CUDA。")
    ...
    try:
        ...
        for fold_id,(ps,pu) in enumerate(folds):
            ...
            package["prototypes"]=ff; package["cra"]=fold_cra; package["visual"]=fold_visual; package["seen_mask"]=torch.isin(allclasses,ps).to(device); packages.append(package); del fold_model
        fusion=NormalizedGeometricVisualFusion().to(device); optimizer=torch.optim.Adam(fusion.parameters(),lr=float(config["lr"]),weight_decay=0.0); identity=SymmetricDiagonalMetric().to(device); identity.raw_log_weight.requires_grad_(False); mapping=torch.full((200,),-1,dtype=torch.long); mapping[seenclasses]=torch.arange(150); generators=[torch.Generator(device="cpu").manual_seed(seed*51000+i) for i in range(3)]; half=int(config["batch_half"]); history=[]; input_sha.update(h1.verify_inputs(base_config,paths,h1.OFFICIAL_KEYS)); tensors.update({name:torch.load(paths[name],map_location="cpu",weights_only=True) for name in h1.OFFICIAL_KEYS}); initial=evaluate(final_p,parent.scale(),main_cra,main_visual,beta,gamma,fusion,tensors,seenclasses,unseenclasses,device); best_H=initial["H"]; best_epoch=0; best_state=copy.deepcopy(fusion.state_dict()); history.append({"epoch":0,"official_metrics_percent":initial,"eta":0.0}); print(f"epoch=0 official_H={best_H:.6f} eta=0")
        for epoch in range(1,int(config["epochs"])+1):
            ...
            for fold_id,package in enumerate(packages):
                ...
                for _ in range(steps):
                    gen=generators[fold_id]; si=package["seen_indices"][torch.randperm(package["seen_indices"].numel(),generator=gen)[:half]]; ui=package["unseen_indices"][torch.randperm(package["unseen_indices"].numel(),generator=gen)[:half]]; indices=torch.cat((si,ui)); images=tensors["train_features"][indices].to(device).float(); targets=mapping[labels[indices]].to(device); additive,normalized=paired_logits(images,package["prototypes"],package["scale"].to(device),package["cra"],package["visual"],beta,gamma,package["seen_mask"],identity,seenclasses); logits=fusion(additive,normalized); loss=F.cross_entropy(logits,targets); optimizer.zero_grad(set_to_none=True); loss.backward(); optimizer.step(); loss_sum+=float(loss.detach()); count+=1
            official=evaluate(final_p,parent.scale(),main_cra,main_visual,beta,gamma,fusion,tensors,seenclasses,unseenclasses,device); row={"epoch":epoch,"loss":loss_sum/count,"official_metrics_percent":official,"eta":float(fusion.eta().detach())}; history.append(row); print(f"epoch={epoch} loss={row['loss']:.6f} official_H={official['H']:.6f} eta={row['eta']:.6f}")
            if official["H"]>best_H: best_H=official["H"]; best_epoch=epoch; best_state=copy.deepcopy(fusion.state_dict())
        fusion.load_state_dict(best_state,strict=True); torch.save({"attempt_id":config["attempt_id"],"code_commit":code_commit,"config":config,"selected_epoch":best_epoch,"ngvf_state_dict":best_state,"history":history},output_dir/"ngvf_model.pth"); parent_fusion=NormalizedGeometricVisualFusion().to(device); parent_metrics=evaluate(final_p,parent.scale(),main_cra,main_visual,beta,gamma,parent_fusion,tensors,seenclasses,unseenclasses,device); candidate_metrics=evaluate(final_p,parent.scale(),main_cra,main_visual,beta,gamma,fusion,tensors,seenclasses,unseenclasses,device); delta={key:candidate_metrics[key]-float(config["parent_metrics_percent"][key]) for key in ("U","S","H","ZS")}; eta=float(fusion.eta().detach()); success=candidate_metrics["H"]>80.4827675277986 and delta["U"]>=-2 and delta["S"]>=-2 and 0<eta<0.98; atomic_write_json(output_dir/"data_fingerprints.json",{"files":input_sha}); metrics={"attempt_id":config["attempt_id"],"idea_id":config["idea_id"],"framework_id":config["framework_id"],"code_commit":code_commit,"config_sha256":config_sha,"base_config_sha256":base_config_sha,"evaluation_protocol":h1.EVALUATION_PROTOCOL,"test_used_for_selection":True,"unseen_images_used_for_gradient":False,"selected_epoch":best_epoch,"learned_eta":eta,"recomputed_parent_metrics_percent":parent_metrics,"parent_metrics_percent":config["parent_metrics_percent"],"candidate_metrics_percent":candidate_metrics,"delta_vs_parent_percent_points":delta,"success":success,"ngvf_model_sha256":sha256_file(output_dir/"ngvf_model.pth")}; atomic_write_json(output_dir/"metrics.json",metrics); print(metrics); return metrics
    finally:
