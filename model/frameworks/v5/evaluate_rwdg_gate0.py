"""Strict Gate-0 evaluator for IDEA-193 / RWDG.
...
SCHEMA = "gzsl-paper.v5-rwdg-gate0-eval.v1"
CHECKPOINT_SCHEMA = "gzsl-paper.v5-rwdg-gate0-train.v1"
...
class RWDGEvalError(RuntimeError):
    """Raised when Gate-0 evaluation would violate the RWDG contract."""
...
def load_config(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if invalid:
        raise RWDGEvalError("RWDG Gate0 eval固定协议错误。")
...
def load_checkpoint(
    ...
    *,
    ...
    if not path.is_file() or sha256_file(path) != str(spec["sha256"]):
        raise RWDGEvalError("RWDG checkpoint路径或SHA错误。")
    checkpoint = _torch_load(path)
    if not isinstance(checkpoint, Mapping):
        raise RWDGEvalError("RWDG checkpoint不是mapping。")
    if expected_train_config_sha256 is not None and spec["train_config_sha256"] != expected_train_config_sha256:
        raise RWDGEvalError("RWDG checkpoint train config SHA与预期不一致。")
    invalid = (
        checkpoint.get("schema_version") != CHECKPOINT_SCHEMA
        or checkpoint.get("condition_id") != "RWDG_FULL"
    ...
    if invalid:
        raise RWDGEvalError("RWDG checkpoint身份错误。")
    return checkpoint
...
def train_static_best_action(
    ...
    *,
    ...
    for start in range(0, train_view.size, batch_size):
        ...
        targets, _, _ = model.dense_utility_targets(
            batch["cls"],
            crop_batch,
            labels[start:end].to(device),
        )
        sums += targets.double().sum(dim=0).cpu()
        all_zero += int(targets.eq(0).all(dim=1).sum().cpu())
    ...
    return action, [float(x) for x in sums], _json_sha256(receipt)
...
def run(config_path: Path, output_dir: Path, expected_commit: str, expected_config_sha: str) -> dict[str, Any]:
    ...
    if code_commit != expected_commit:
        raise RWDGEvalError("RWDG eval expected commit mismatch.")
    config, config_sha = load_config(config_path)
    if config_sha != expected_config_sha:
        raise RWDGEvalError("RWDG eval config SHA mismatch.")
    ...
    if eval_view.size != 2355 or train_view.size != 4702 or eval_class_ids.numel() != 150 or train_class_ids.numel() != 100:
        raise RWDGEvalError("RWDG Gate0 train/eval row或class数量错误。")
    ...
    static_action, static_sums, static_sha = train_static_best_action(
        train_model,
        train_view,
        assets,
        train_class_ids,
    ...
    result = {
        ...
        "decision": "continue_gate1_controls" if passed else "drop_rwdg_gate0_failed",
        "transitions": {
            "corrected": int(corrected.sum()),
            "damaged": int(damaged.sum()),
            "net": int(corrected.sum() - damaged.sum()),
        },
        "group_statistics": group_statistics(full_trace, labels, eval_class_ids, parent_pred, full_pred),
        "utility": {
