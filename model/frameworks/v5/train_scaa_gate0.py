"""Gate-0 Full-only trainer for IDEA-195 / SCAA.

The script is deliberately narrow:

* trains only the Full SCAA signed action-advantage objective for the fixed Gate-0 budget;
...
TRAIN_SCHEMA = "gzsl-paper.v5-scaa-gate0-train.v1"


SCAA_PREREGISTERED_TARGET_COUNTS = {
    "total_rows": 4702,
    "leader_rows": 3478,
    "challenger_rows": 634,
    "outside_rows": 590,
    "positive_actions": 6279,
    "negative_actions": 35262,
    "zero_actions": 76009,
}
...
def validate_config(config: Gate0TrainConfig) -> None:
    ...
    if config.condition_id != "SCAA_FULL":
        raise RWDGDataError("Gate0 training condition_id must be SCAA_FULL")
...
def scaa_signed_action_advantage_targets(
    ...
    *,
    ...
    signed_targets = torch.zeros_like(dense_targets)
    leader_rows = groups == 0
    challenger_rows = groups == 1
    signed_targets[leader_rows] = dense_targets[leader_rows] - 1.0
    signed_targets[challenger_rows] = dense_targets[challenger_rows]
    return signed_targets, groups, pair


# Public contract name used by tests and downstream controls.
scaa_signed_targets = scaa_signed_action_advantage_targets


def signed_advantage_loss(utility_logits: Tensor, targets: Tensor) -> Tensor:
    if utility_logits.shape != targets.shape:
        raise ValueError(
            f"SCAA utility logits/targets shape mismatch: logits={tuple(utility_logits.shape)} "
            f"targets={tuple(targets.shape)}"
        )
    if utility_logits.ndim != 2 or utility_logits.shape[1] != ACTION_COUNT:
        raise ValueError(f"SCAA utility logits must have shape [B, {ACTION_COUNT}]")
    return F.mse_loss(torch.tanh(utility_logits), targets.float())
...
def _sign_counts(targets: Tensor) -> dict[str, int]:
    return {
        "positive": int((targets > 0).sum().item()),
        "negative": int((targets < 0).sum().item()),
        "zero": int((targets == 0).sum().item()),
    }


def _per_action_sign_counts(targets: Tensor) -> dict[str, list[int]]:
    return {
        "positive": [int(x) for x in (targets > 0).sum(dim=0).tolist()],
        "negative": [int(x) for x in (targets < 0).sum(dim=0).tolist()],
        "zero": [int(x) for x in (targets == 0).sum(dim=0).tolist()],
    }


def _per_group_sign_counts(targets: Tensor, groups: Tensor) -> dict[str, dict[str, int]]:
    names = ("leader", "challenger", "outside")
    out: dict[str, dict[str, int]] = {}
    for group_id, name in enumerate(names):
        mask = groups == group_id
        if bool(mask.any()):
            out[name] = _sign_counts(targets[mask])
        else:
            out[name] = {"positive": 0, "negative": 0, "zero": 0}
    return out


def precompute_scaa_target_plan(
    model: RoleWindowDenseGlimpse,
    train_view: RWDGGateSubsetView,
    targets: TrainSubsetTargets,
    *,
    ...
    with torch.no_grad():
        for start in range(0, train_view.size, batch_size):
            ...
            signed_targets, groups, _ = scaa_signed_action_advantage_targets(
                model,
                full_cls,
                crops,
                labels,
            )
            signed_targets_cpu = signed_targets.detach().cpu().float()
            groups_cpu = groups.detach().cpu().long()
            bad_values = ~((signed_targets_cpu == -1.0) | (signed_targets_cpu == 0.0) | (signed_targets_cpu == 1.0))
            if bool(bad_values.any()):
                raise RWDGDataError("SCAA signed targets must be exactly -1, 0, or +1")
            all_groups.append(groups_cpu)
            all_targets.append(signed_targets_cpu)

    groups = torch.cat(all_groups).long()
    signed_targets = torch.cat(all_targets).float()
    group_counts = torch.bincount(groups, minlength=3)[:3].long()
    sign_counts = _sign_counts(signed_targets)
    group_sign_counts = _per_group_sign_counts(signed_targets, groups)
    total_rows = int(groups.numel())
    total_actions = max(total_rows * ACTION_COUNT, 1)
    challenger_rows = int(group_counts[1].item())
    stats = {
        "target_definition": "scaa_signed_action_advantage",
        ...
        "target_density": float((sign_counts["positive"] + sign_counts["negative"]) / total_actions),
        "sign_density": {
            "positive": float(sign_counts["positive"] / total_actions),
            "negative": float(sign_counts["negative"] / total_actions),
            "zero": float(sign_counts["zero"] / total_actions),
        },
        "per_action_sign_counts": _per_action_sign_counts(signed_targets),
        "per_group_sign_counts": group_sign_counts,
        "mean_signed_target": [float(x) for x in signed_targets.double().mean(dim=0).tolist()],
    }
    assert_pre_registered_scaa_counts(stats)
    return SCAATargetPlan(targets=signed_targets, groups=groups, stats=stats)


def assert_pre_registered_scaa_counts(stats: Mapping[str, Any]) -> None:
    natural = stats.get("natural_distribution")
    if not isinstance(natural, Mapping):
        raise RWDGDataError("SCAA target stats missing natural_distribution")
    groups = natural.get("group_counts")
    if not isinstance(groups, Mapping):
        raise RWDGDataError("SCAA target stats missing group_counts")
    sign_counts = stats.get("sign_counts")
    per_group = stats.get("per_group_sign_counts")
    per_action = stats.get("per_action_sign_counts")
    if not isinstance(sign_counts, Mapping):
        raise RWDGDataError("SCAA target stats missing sign_counts")
    if not isinstance(per_group, Mapping):
        raise RWDGDataError("SCAA target stats missing per_group_sign_counts")
    if not isinstance(per_action, Mapping):
        raise RWDGDataError("SCAA target stats missing per_action_sign_counts")
    ...
    actual = {
        ...
        "positive_actions": int(sign_counts.get("positive", -1)),
        "negative_actions": int(sign_counts.get("negative", -1)),
        "zero_actions": int(sign_counts.get("zero", -1)),
    }
    mismatches = {
        key: {"actual": actual[key], "expected": expected}
        for key, expected in SCAA_PREREGISTERED_TARGET_COUNTS.items()
        if actual[key] != expected
    }
    leader_counts = per_group.get("leader", {})
    challenger_counts = per_group.get("challenger", {})
    outside_counts = per_group.get("outside", {})
    if int(leader_counts.get("positive", -1)) != 0 or int(outside_counts.get("positive", -1)) != 0:
        mismatches["positive_actions_not_challenger"] = {
            "actual": {
                "leader": int(leader_counts.get("positive", -1)),
                "outside": int(outside_counts.get("positive", -1)),
            },
            "expected": {"leader": 0, "outside": 0},
        }
    if int(challenger_counts.get("negative", -1)) != 0 or int(outside_counts.get("negative", -1)) != 0:
        mismatches["negative_actions_not_leader"] = {
            "actual": {
                "challenger": int(challenger_counts.get("negative", -1)),
                "outside": int(outside_counts.get("negative", -1)),
            },
            "expected": {"challenger": 0, "outside": 0},
        }
    if mismatches:
        raise RWDGDataError(f"SCAA preregistered target counts mismatch: {mismatches}")
    ...
    def __init__(
        ...
        *,
        ...
        if batch_size != 8 or challenger_per_batch != 4:
            raise RWDGDataError("SCAA sampler requires batch8 with 4 challenger rows")
        ...
        if challenger.numel() < challenger_per_batch:
            raise RWDGDataError("SCAA sampler has too few challenger rows")
        if non_challenger.numel() < batch_size - challenger_per_batch:
            raise RWDGDataError("SCAA sampler has too few non-challenger rows")
...
def sampled_sign_totals(
    ...
    *,
    ...
    for _ in range(int(updates)):
        rows = sampler.sample()
        batch_targets = targets.index_select(0, rows)
        batch_counts = _sign_counts(batch_targets)
        for key, value in batch_counts.items():
            counts[key] += int(value)
    if counts["positive"] <= 0 or counts["negative"] <= 0:
        raise RWDGDataError(f"SCAA sampled sign totals must include both +/- targets: {counts}")
    total_actions = max(int(updates) * int(batch_size) * ACTION_COUNT, 1)
    return {
        "updates": int(updates),
        "batch_size": int(batch_size),
        "sampled_actions": total_actions,
        "sign_counts": counts,
        "sign_density": {key: float(value / total_actions) for key, value in counts.items()},
...
def train_step(
    ...
    *,
    ...
    target, _, _ = scaa_signed_action_advantage_targets(model, full_cls, crops, labels)
    utility = model.utility_state(full_cls, patches)
    loss = signed_advantage_loss(utility.utility_logits, target)
    loss.backward()
...
def signed_static_best_action_from_natural_targets(targets: Tensor) -> tuple[int, Mapping[str, Any]]:
    if targets.ndim != 2 or targets.shape[1] != ACTION_COUNT:
        raise RWDGDataError(f"natural SCAA signed targets must have shape [N,{ACTION_COUNT}]")
    signed_targets = targets.detach().cpu().float()
    sign_counts = _sign_counts(signed_targets)
    mean_signed = signed_targets.double().mean(dim=0)
    action = int(torch.argmax(mean_signed).item())
    return action, {
        "count": int(targets.shape[0]),
        "mean_signed_target": [float(x) for x in mean_signed.tolist()],
        "sign_counts": sign_counts,
        "target_density": float((sign_counts["positive"] + sign_counts["negative"]) / max(targets.numel(), 1)),
    }


static_best_action_from_natural_targets = signed_static_best_action_from_natural_targets
    ...
    *,
    ...
    with torch.no_grad():
        for start in range(0, train_view.size, batch_size):
            rows = torch.arange(start, min(start + batch_size, train_view.size), dtype=torch.long)
            full_cls, _, crops, labels = batch_from_rows(train_view, targets, rows, device=device)
            signed_targets, groups, _ = scaa_signed_action_advantage_targets(model, full_cls, crops, labels)
            signed_targets_cpu = signed_targets.detach().cpu().float()
            target_sum += signed_targets_cpu.double().sum(dim=0)
            total += int(signed_targets_cpu.shape[0])
            group_counts += torch.bincount(groups.detach().cpu(), minlength=3)[:3]
            all_targets.append(signed_targets_cpu)
    signed_targets_all = torch.cat(all_targets).float() if all_targets else torch.empty(0, ACTION_COUNT)
    sign_counts = _sign_counts(signed_targets_all)
    mean_signed = target_sum / max(total, 1)
    static_best = int(torch.argmax(mean_signed).item())  # torch.argmax picks smallest tie.
    return {
        "static_best_action": static_best,
        "static_best_rule": "argmax natural mean signed target",
        "mean_signed_target": [float(x) for x in mean_signed.tolist()],
        "sign_counts": sign_counts,
        "target_density": float((sign_counts["positive"] + sign_counts["negative"]) / max(total * ACTION_COUNT, 1)),
        "per_action_sign_counts": _per_action_sign_counts(signed_targets_all),
        "group_counts": {
            "leader": int(group_counts[0].item()),
...
def train_full_gate0(
    ...
    *,
    ...
    try:
        ...
        target_plan = precompute_scaa_target_plan(
            model,
            train_view,
            train_targets,
        ...
        sampled_stats = sampled_sign_totals(
            target_plan.targets,
            target_plan.groups,
        ...
        static_best_action, static_best_detail = signed_static_best_action_from_natural_targets(target_plan.targets)
        static_best = {
            ...
            **dict(static_best_detail),
            ...
            "per_action_sign_counts": target_plan.stats["per_action_sign_counts"],
        }
        ...
        checkpoint_path = output_dir / "scaa_gate0_full.pt"
        ...
        atomic_write_json(
            history_path,
            {
                ...
                "sampled_sign_stats": checkpoint["sampled_sign_stats"],
            },
        )
    ...
    *,
    ...
    return {
        ...
        "reproducibility_identity": {
            ...
            "weight_decay": config.weight_decay,
            "sampler": sampler.state_dict(),
            "sampled_sign_stats": sampled_stats,
...
__all__ = [
    ...
    "prepare_output_dir",
    "precompute_scaa_target_plan",
    "sampled_sign_totals",
    "set_reproducibility",
    "scaa_signed_action_advantage_targets",
    "scaa_signed_targets",
    "signed_advantage_loss",
    "signed_static_best_action_from_natural_targets",
