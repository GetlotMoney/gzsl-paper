"""Strict Gate-0 evaluator for IDEA-195 / SCAA.
...
from model.frameworks.v5.rwdg_data import (
    ManifestContract,
    RWDGAssetConfig,
    RWDGDataError,
    RWDGGateSubsetView,
...
SCHEMA = "gzsl-paper.v5-scaa-gate0-eval.v1"
CHECKPOINT_SCHEMA = "gzsl-paper.v5-scaa-gate0-train.v1"
...
class RWDGEvalError(RuntimeError):
    """Raised when Gate-0 evaluation would violate the SCAA contract."""
...
def load_config(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if invalid:
        raise RWDGEvalError("SCAA Gate0 eval固定协议错误。")
...
def load_checkpoint(
    ...
    *,
    ...
    if not path.is_file() or sha256_file(path) != str(spec["sha256"]):
        raise RWDGEvalError("SCAA checkpoint路径或SHA错误。")
    checkpoint = _torch_load(path)
    if not isinstance(checkpoint, Mapping):
        raise RWDGEvalError("SCAA checkpoint不是mapping。")
    if expected_train_config_sha256 is not None and spec["train_config_sha256"] != expected_train_config_sha256:
        raise RWDGEvalError("SCAA checkpoint train config SHA与预期不一致。")
    invalid = (
        checkpoint.get("schema_version") != CHECKPOINT_SCHEMA
        or checkpoint.get("condition_id") != "SCAA_FULL"
    ...
    if invalid:
        raise RWDGEvalError("SCAA checkpoint身份错误。")
    return checkpoint
...
def freeze_decisions(
    ...
    *,
    ...
    attention_mass: list[torch.Tensor] = []
    for start in range(0, view.size, batch_size):
        ...
        att = state.attention.detach().cpu().float()
        ...
        attention_entropy.append((-(att * att.clamp_min(1e-12).log()).sum(dim=2)))
        attention_mass.append(state.attention_mass.detach().cpu().float())
    return FrozenDecisions(
        ...
        attention_entropy=torch.cat(attention_entropy).float(),
        attention_mass=torch.cat(attention_mass).float(),
    )
...
def train_static_best_action(
    ...
    *,
    device: torch.device,
    batch_size: int,
) -> tuple[int, list[float], str, dict[str, float | int]]:
    ...
    sums = torch.zeros(ACTION_COUNT, dtype=torch.double)
    all_zero = 0
    signed_min = float("inf")
    signed_max = float("-inf")
    signed_sum = 0.0
    signed_count = 0
    near_zero_positive = 0
    for start in range(0, train_view.size, batch_size):
        ...
        signed_targets = torch.zeros_like(targets)
        signed_targets[group.eq(0)] = targets[group.eq(0)] - 1.0
        signed_targets[group.eq(1)] = targets[group.eq(1)]
        sums += signed_targets.double().sum(dim=0).cpu()
        all_zero += int(signed_targets.eq(0).all(dim=1).sum().cpu())
        flat = signed_targets.detach().cpu().double().flatten()
        signed_min = min(signed_min, float(flat.min()))
        signed_max = max(signed_max, float(flat.max()))
        signed_sum += float(flat.sum())
        signed_count += int(flat.numel())
        near_zero_positive += int(((flat > 0) & (flat <= 1e-6)).sum())
    action = int(torch.argmax(sums).item())
    signed_stats = {
        "min": signed_min,
        "mean": signed_sum / max(1, signed_count),
        "max": signed_max,
        "near_zero_positive_count": near_zero_positive,
    }
    receipt = {
        "sums": [float(x) for x in sums],
        "all_zero_rows": int(all_zero),
        "count": int(train_view.size),
        "target_policy": "signed_leader_old_minus1_challenger_old_outside0",
        "natural_train_rows": int(train_view.size),
        "signed_advantage": signed_stats,
    }
    return action, [float(x) for x in sums], _json_sha256(receipt), signed_stats
...
def attention_summary(trace: FrozenDecisions) -> dict[str, Any]:
    mass = trace.attention_mass.double()
    entropy = trace.attention_entropy.double()
    return {
        "mean_role_entropy": float(entropy.mean()),
        "per_role_entropy": [float(x) for x in entropy.mean(dim=0)],
        "mean_action_mass": [float(x) for x in mass.mean(dim=0)],
        "max_action_mass_mean": float(mass.max(dim=1).values.mean()),
    }
...
def run(config_path: Path, output_dir: Path, expected_commit: str, expected_config_sha: str) -> dict[str, Any]:
    ...
    if code_commit != expected_commit:
        raise RWDGEvalError("SCAA eval expected commit mismatch.")
    config, config_sha = load_config(config_path)
    if config_sha != expected_config_sha:
        raise RWDGEvalError("SCAA eval config SHA mismatch.")
    ...
    if eval_view.size != 2355 or train_view.size != 4702 or eval_class_ids.numel() != 150 or train_class_ids.numel() != 100:
        raise RWDGEvalError("SCAA Gate0 train/eval row或class数量错误。")
    ...
    static_action, static_sums, static_sha, signed_advantage = train_static_best_action(
        train_model,
        train_view,
        assets,
        train_class_ids,
    ...
    for name in controls:
        control_trace = FrozenDecisions(
            ...
            attention_entropy=full_trace.attention_entropy,
            attention_mass=full_trace.attention_mass,
        )
    ...
    result = {
        ...
        "decision": "continue_gate1_controls" if passed else "drop_scaa_gate0_failed",
        "transitions": {
        ...
        "attention": attention_summary(full_trace),
        "controls": {
            "static_best_action": int(static_action),
            "static_best_train_sums": static_sums,
            "static_best_sha256": static_sha,
            "static_best_target_policy": "signed_leader_old_minus1_challenger_old_outside0",
