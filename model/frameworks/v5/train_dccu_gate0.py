"""Gate-0 Full-only trainer for IDEA-194 / D-CCU.

The script is deliberately narrow:

* trains only the Full D-CCU dense correction objective for the fixed Gate-0 budget;
...
TRAIN_SCHEMA = "gzsl-paper.v5-dccu-gate0-train.v1"


DCCU_PREREGISTERED_TARGET_COUNTS = {
    "total_rows": 4702,
    "leader_rows": 3478,
    "challenger_rows": 634,
    "outside_rows": 590,
    "positive_actions": 6279,
    "positive_challenger_rows": 595,
    "challenger_all_zero_rows": 39,
}
...
def validate_config(config: Gate0TrainConfig) -> None:
    ...
    if config.condition_id != "DCCU_FULL":
        raise RWDGDataError("Gate0 training condition_id must be DCCU_FULL")
...
def dccu_dense_correction_targets(
    ...
    *,
    ...
    correction_targets = torch.zeros_like(dense_targets)
    challenger_rows = groups == 1
    correction_targets[challenger_rows] = dense_targets[challenger_rows]
    return correction_targets, groups, pair


# Public contract name used by tests and downstream controls.
dccu_correction_targets = dccu_dense_correction_targets
...
def precompute_dccu_target_plan(
    model: RoleWindowDenseGlimpse,
    train_view: RWDGGateSubsetView,
    targets: TrainSubsetTargets,
    *,
    ...
    positive_rows_by_group = torch.zeros(3, dtype=torch.long)
    positive_actions_by_group = torch.zeros(3, dtype=torch.long)
    all_zero_rows_by_group = torch.zeros(3, dtype=torch.long)
    with torch.no_grad():
        for start in range(0, train_view.size, batch_size):
            ...
            dense_targets, groups, _ = dccu_dense_correction_targets(
                model,
                full_cls,
                crops,
                labels,
            )
            dense_targets_cpu = dense_targets.detach().cpu()
            groups_cpu = groups.detach().cpu().long()
            row_positive_counts = dense_targets_cpu.long().sum(dim=1)
            all_groups.append(groups_cpu)
            target_sum += dense_targets_cpu.double().sum(dim=0)
            for group_id in range(3):
                mask = groups_cpu == group_id
                if bool(mask.any()):
                    group_row_positive_counts = row_positive_counts[mask]
                    positive_rows_by_group[group_id] += int((group_row_positive_counts > 0).sum().item())
                    positive_actions_by_group[group_id] += int(group_row_positive_counts.sum().item())
                    all_zero_rows_by_group[group_id] += int((group_row_positive_counts == 0).sum().item())

    groups = torch.cat(all_groups).long()
    group_counts = torch.bincount(groups, minlength=3)[:3].long()
    positive_actions = int(target_sum.sum().item())
    total_rows = int(groups.numel())
    challenger_rows = int(group_counts[1].item())
    stats = {
        "target_definition": "dccu_correction_only",
        ...
        "target_density": float(positive_actions / max(total_rows * ACTION_COUNT, 1)),
        "positive_rows_by_group": {
            "leader": int(positive_rows_by_group[0].item()),
            "challenger": int(positive_rows_by_group[1].item()),
            "outside": int(positive_rows_by_group[2].item()),
        },
        "positive_actions_by_group": {
            "leader": int(positive_actions_by_group[0].item()),
            "challenger": int(positive_actions_by_group[1].item()),
            "outside": int(positive_actions_by_group[2].item()),
        },
        "all_zero_rows_by_group": {
            "leader": int(all_zero_rows_by_group[0].item()),
            "challenger": int(all_zero_rows_by_group[1].item()),
            "outside": int(all_zero_rows_by_group[2].item()),
        },
        "challenger_positive_density": float(
            positive_actions_by_group[1].item() / max(challenger_rows * ACTION_COUNT, 1)
        ),
        "all_zero_rate": float(sum(int(x) for x in all_zero_rows_by_group.tolist()) / max(total_rows, 1)),
        "mean_target_utility": [float(x) for x in (target_sum / max(total_rows, 1)).tolist()],
    }
    assert_pre_registered_dccu_counts(stats)
    return DCCUTargetPlan(groups=groups, stats=stats)


def assert_pre_registered_dccu_counts(stats: Mapping[str, Any]) -> None:
    natural = stats.get("natural_distribution")
    if not isinstance(natural, Mapping):
        raise RWDGDataError("D-CCU target stats missing natural_distribution")
    groups = natural.get("group_counts")
    if not isinstance(groups, Mapping):
        raise RWDGDataError("D-CCU target stats missing group_counts")
    positives_by_group = stats.get("positive_actions_by_group")
    positive_rows_by_group = stats.get("positive_rows_by_group")
    all_zero_rows_by_group = stats.get("all_zero_rows_by_group")
    if not isinstance(positives_by_group, Mapping):
        raise RWDGDataError("D-CCU target stats missing positive_actions_by_group")
    if not isinstance(positive_rows_by_group, Mapping):
        raise RWDGDataError("D-CCU target stats missing positive_rows_by_group")
    if not isinstance(all_zero_rows_by_group, Mapping):
        raise RWDGDataError("D-CCU target stats missing all_zero_rows_by_group")
    ...
    actual = {
        ...
        "positive_actions": int(stats.get("positive_actions", -1)),
        "positive_challenger_rows": int(positive_rows_by_group.get("challenger", -1)),
        "challenger_all_zero_rows": int(all_zero_rows_by_group.get("challenger", -1)),
    }
    mismatches = {
        key: {"actual": actual[key], "expected": expected}
        for key, expected in DCCU_PREREGISTERED_TARGET_COUNTS.items()
        if actual[key] != expected
    }
    if int(positives_by_group.get("leader", -1)) != 0 or int(positives_by_group.get("outside", -1)) != 0:
        mismatches["non_challenger_positive_actions"] = {
            "actual": {
                "leader": int(positives_by_group.get("leader", -1)),
                "outside": int(positives_by_group.get("outside", -1)),
            },
            "expected": {"leader": 0, "outside": 0},
        }
    if mismatches:
        raise RWDGDataError(f"D-CCU preregistered target counts mismatch: {mismatches}")
    ...
    def __init__(
        ...
        *,
        ...
        if batch_size != 8 or challenger_per_batch != 4:
            raise RWDGDataError("D-CCU sampler requires batch8 with 4 challenger rows")
        ...
        if challenger.numel() < challenger_per_batch:
            raise RWDGDataError("D-CCU sampler has too few challenger rows")
        if non_challenger.numel() < batch_size - challenger_per_batch:
            raise RWDGDataError("D-CCU sampler has too few non-challenger rows")
        ...
        self._challenger_gen = torch.Generator(device="cpu").manual_seed(int(seed))
        self._non_challenger_gen = torch.Generator(device="cpu").manual_seed(int(seed))
        self._batch_gen = torch.Generator(device="cpu").manual_seed(int(seed))
...
def train_step(
    ...
    *,
    ...
    target, _, _ = dccu_dense_correction_targets(model, full_cls, crops, labels)
...
def static_best_action_from_natural_targets(targets: Tensor) -> tuple[int, Mapping[str, Any]]:
    if targets.ndim != 2 or targets.shape[1] != ACTION_COUNT:
        raise RWDGDataError("natural D-CCU targets must have shape [N,25]")
    mean_utility = targets.detach().cpu().double().mean(dim=0)
    action = int(torch.argmax(mean_utility).item())
    return action, {
    ...
    *,
    ...
    with torch.no_grad():
        for start in range(0, train_view.size, batch_size):
            rows = torch.arange(start, min(start + batch_size, train_view.size), dtype=torch.long)
            full_cls, _, crops, labels = batch_from_rows(train_view, targets, rows, device=device)
            dense_targets, groups, _ = dccu_dense_correction_targets(model, full_cls, crops, labels)
...
def train_full_gate0(
    ...
    *,
    ...
    try:
        ...
        target_plan = precompute_dccu_target_plan(
            model,
            train_view,
            train_targets,
        ...
        checkpoint_path = output_dir / "dccu_gate0_full.pt"
