"""Strict Gate-0 evaluator for IDEA-194 / D-CCU.
...
SCHEMA = "gzsl-paper.v5-dccu-gate0-eval.v1"
CHECKPOINT_SCHEMA = "gzsl-paper.v5-dccu-gate0-train.v1"
...
class RWDGEvalError(RuntimeError):
    """Raised when Gate-0 evaluation would violate the DCCU contract."""
...
def load_config(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if invalid:
        raise RWDGEvalError("DCCU Gate0 eval固定协议错误。")
...
def load_checkpoint(
    ...
    *,
    ...
    if not path.is_file() or sha256_file(path) != str(spec["sha256"]):
        raise RWDGEvalError("DCCU checkpoint路径或SHA错误。")
    checkpoint = _torch_load(path)
    if not isinstance(checkpoint, Mapping):
        raise RWDGEvalError("DCCU checkpoint不是mapping。")
    if expected_train_config_sha256 is not None and spec["train_config_sha256"] != expected_train_config_sha256:
        raise RWDGEvalError("DCCU checkpoint train config SHA与预期不一致。")
    invalid = (
        checkpoint.get("schema_version") != CHECKPOINT_SCHEMA
        or checkpoint.get("condition_id") != "DCCU_FULL"
    ...
    if invalid:
        raise RWDGEvalError("DCCU checkpoint身份错误。")
    return checkpoint
...
def train_static_best_action(
    ...
    *,
    ...
    for start in range(0, train_view.size, batch_size):
        ...
        targets = targets.masked_fill(group.ne(1).unsqueeze(1), 0.0)
    ...
    receipt = {
        ...
        "target_policy": "mask_group_not_challenger_to_zero",
...
def run(config_path: Path, output_dir: Path, expected_commit: str, expected_config_sha: str) -> dict[str, Any]:
    ...
    if code_commit != expected_commit:
        raise RWDGEvalError("DCCU eval expected commit mismatch.")
    config, config_sha = load_config(config_path)
    if config_sha != expected_config_sha:
        raise RWDGEvalError("DCCU eval config SHA mismatch.")
    ...
    if eval_view.size != 2355 or train_view.size != 4702 or eval_class_ids.numel() != 150 or train_class_ids.numel() != 100:
        raise RWDGEvalError("DCCU Gate0 train/eval row或class数量错误。")
    ...
    result = {
        ...
        "decision": "continue_gate1_controls" if passed else "drop_dccu_gate0_failed",
        "transitions": {
