"""Strict Gate-0 evaluator for IDEA-196 / EAAC.
...
SCHEMA = "gzsl-paper.v5-eaac-gate0-eval.v1"
CHECKPOINT_SCHEMA = "gzsl-paper.v5-eaac-gate0-train.v1"
...
class RWDGEvalError(RuntimeError):
    """Raised when Gate-0 evaluation would violate the EAAC contract."""
...
def load_config(path: Path) -> tuple[dict[str, Any], str]:
    ...
    if invalid:
        raise RWDGEvalError("EAAC Gate0 eval固定协议错误。")
...
def load_checkpoint(
    ...
    *,
    ...
    if not path.is_file() or sha256_file(path) != str(spec["sha256"]):
        raise RWDGEvalError("EAAC checkpoint路径或SHA错误。")
    checkpoint = _torch_load(path)
    if not isinstance(checkpoint, Mapping):
        raise RWDGEvalError("EAAC checkpoint不是mapping。")
    if expected_train_config_sha256 is not None and spec["train_config_sha256"] != expected_train_config_sha256:
        raise RWDGEvalError("EAAC checkpoint train config SHA与预期不一致。")
    invalid = (
        checkpoint.get("schema_version") != CHECKPOINT_SCHEMA
        or checkpoint.get("condition_id") != "EAAC_FULL"
    ...
    if invalid:
        raise RWDGEvalError("EAAC checkpoint身份错误。")
    return checkpoint
...
def train_static_best_action(
    ...
    *,
    ...
    choice_stats = {
        "target_policy": "eaac26_abstain_or_strongest_negative_margin",
...
def run(config_path: Path, output_dir: Path, expected_commit: str, expected_config_sha: str) -> dict[str, Any]:
    ...
    if code_commit != expected_commit:
        raise RWDGEvalError("EAAC eval expected commit mismatch.")
    config, config_sha = load_config(config_path)
    if config_sha != expected_config_sha:
        raise RWDGEvalError("EAAC eval config SHA mismatch.")
    ...
    if eval_view.size != 2355 or train_view.size != 4702 or eval_class_ids.numel() != 150 or train_class_ids.numel() != 100:
        raise RWDGEvalError("EAAC Gate0 train/eval row或class数量错误。")
    ...
    result = {
        ...
        "decision": "continue_gate1_controls" if passed else "drop_eaac_gate0_failed",
        "transitions": {
        ...
        "controls": {
            ...
            "static_best_target_policy": "eaac26_abstain_or_strongest_negative_margin",
