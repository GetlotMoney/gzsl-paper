"""BRPL rescue training on the fixed CTPM inference architecture."""
...
SCHEMA = "gzsl-paper.v6-brpl-train.v1"
RESULT_SCHEMA = "gzsl-paper.v6-brpl-result.v1"
CONFIG_KEYS = {
...
def load_config(path: Path) -> tuple[dict[str, Any], str]:
    ...
    invalid = (
        ...
        or config.get("experiment_id") != "V6-TRY-010-R1"
        or config.get("framework_id") != "FRAMEWORK-V6-DEVELOPMENT"
        or config.get("condition_id") != "BRPL_FIXED200"
        ...
        or int(config.get("rescue_index", -1)) != 1
    ...
    if invalid:
        raise ValueError("BRPL config identity, budget, parent, or disclosure mismatch.")
        ...
        for parameter in params:
            if parameter.grad is not None:
                if not torch.isfinite(parameter.grad).all():
                    raise FloatingPointError(f"BRPL non-finite gradient: {name}")
...
def brpl_losses(
    ...
    """Full CE plus V-only and I-only balanced residual pair supervision."""
    full_ce = F.cross_entropy(
        output.logits.index_select(1, seen), global_to_seen[labels]
    )
    prefix_v = (
        output.base_logits + output.role_logits
        + pair_scatter(output.top2_local, output.d_s, output.logits.size(1))
    ).detach()
    v_pair_logits = (
        prefix_v + pair_scatter(output.top2_local, output.d_v, output.logits.size(1))
    ).gather(1, output.top2_local)
    visual_pair, visual_skipped = balanced_pair_ce(v_pair_logits, output.top2_global, labels)

    d_i_isolated = isolated_interaction_margin(model, output)
    prefix_i = (
        output.base_logits + output.role_logits
        + pair_scatter(output.top2_local, output.d_s + output.d_v, output.logits.size(1))
    ).detach()
    i_pair_logits = (
        prefix_i + pair_scatter(output.top2_local, d_i_isolated, output.logits.size(1))
    ).gather(1, output.top2_local)
    interaction_pair, interaction_skipped = balanced_pair_ce(
        i_pair_logits, output.top2_global, labels
    )
    diversity = attention_diversity_loss(output.attention)
    total = full_ce + visual_pair + interaction_pair + float(attention_diversity_weight) * diversity
    return total, {
        "full_ce": full_ce,
...
def first_balanced_pair_rows(model: CTPMModel, assets: CTPMTrainAssets, config) -> tuple[int, torch.Tensor]:
    """Replay the frozen sampling stream until it contains truth=c1 and truth=c2."""
    ...
    for update in range(int(config["total_updates"])):
        ...
        with torch.no_grad():
            ...
            if bool((in_pair & ~target).any()) and bool((in_pair & target).any()):
                return update, rows
    raise RuntimeError("BRPL frozen batch stream never contains both pair targets.")
...
def micro_contract(model: CTPMModel, assets: CTPMTrainAssets, config, device):
    ...
    if any(not np.isfinite(value) or value <= 0 for value in gradients.values()):
        raise RuntimeError(f"BRPL Full CE did not reach every component: {gradients}")
    model.zero_grad(set_to_none=True)
    total, losses = brpl_losses(
        model, output, labels, seen, global_to_seen,
        float(config["attention_diversity_weight"]),
    )
    if bool(losses["visual_skipped"]) or bool(losses["interaction_skipped"]):
        raise RuntimeError("BRPL micro batch unexpectedly lacks one pair target.")
    ...
    if visual_grads["visual"] <= 0 or visual_grads["semantic"] != 0 or visual_grads["interaction"] != 0:
        raise RuntimeError(f"BRPL visual auxiliary isolation failed: {visual_grads}")
    model.zero_grad(set_to_none=True)
    ...
    if interaction_grads["interaction"] <= 0 or interaction_grads["semantic"] != 0 or interaction_grads["visual"] != 0:
        raise RuntimeError(f"BRPL interaction auxiliary isolation failed: {interaction_grads}")
    model.zero_grad(set_to_none=True)
    total.backward()
    with torch.no_grad():
        variants = {
        ...
        for name, kwargs in variants.items():
            ...
            if not torch.isfinite(current.logits).all():
                raise RuntimeError(f"BRPL {name} micro logits non-finite.")
        if not all(torch.equal(pairs["full"], pair_ids) for pair_ids in pairs.values()):
            raise RuntimeError("BRPL pair identity changed across offs.")
        ...
        if correction_sum > 1e-6 or attention_error > 1e-6 or float(output.attention.std()) <= 0:
            raise RuntimeError("BRPL antisymmetry or attention contract failed.")
    model.zero_grad(set_to_none=True)
    return {
...
def run(
    ...
    *,
    ...
    if current_code_commit() != expected_commit:
        raise ValueError("BRPL expected commit mismatch.")
    config, config_sha = load_config(config_path)
    if config_sha != expected_config_sha:
        raise ValueError("BRPL expected config SHA mismatch.")
    ...
    try:
        ...
        if device.type != "cuda" or not torch.cuda.is_available():
            raise RuntimeError("BRPL formal training requires CUDA.")
        ...
        print(f"BRPL commit={expected_commit} config_sha={config_sha}")
        print(json.dumps({"micro": micro}, sort_keys=True))
        if micro_batch_only:
            result = {
                "schema_version": "gzsl-paper.v6-brpl-micro.v1",
        ...
        visual_skips, interaction_skips, train_updates = 0, 0, 0
        ...
        for update in range(int(config["total_updates"]) + 1):
            if update in eval_points:
                ...
                if update == 0 and any(
                    ...
                    raise RuntimeError("BRPL class-name parent did not reproduce gate metrics.")
            ...
            if not torch.isfinite(total):
                raise FloatingPointError("BRPL loss non-finite.")
            total.backward()
            for name, parameter in model.named_parameters():
                if parameter.grad is not None and not torch.isfinite(parameter.grad).all():
                    raise FloatingPointError(f"BRPL gradient non-finite: {name}")
            optimizer.step()
            scheduler.step()

        if best_metrics is None or best_state is None or best_zs is None:
            raise RuntimeError("BRPL did not select a checkpoint.")
        ...
        if any(abs(float(final_metrics[name]) - float(best_metrics[name])) > 1e-6 for name in ("U", "S", "H", "ZS")):
            raise RuntimeError("BRPL best checkpoint metrics did not reproduce.")
        visual_skip_rate = visual_skips / max(train_updates, 1)
        interaction_skip_rate = interaction_skips / max(train_updates, 1)
        if visual_skip_rate > 0.05 or interaction_skip_rate > 0.05:
            raise RuntimeError(
                f"BRPL auxiliary skip rate exceeded 5%: V={visual_skip_rate} I={interaction_skip_rate}"
            )
