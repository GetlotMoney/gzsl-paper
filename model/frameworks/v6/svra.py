...
DIRECT_SWAP_INPUT_DIM: Final[int] = 69
...
@dataclass(frozen=True)
class DirectSVRAOutput:
    """DESC direct keep/swap output."""

    pair: PairState
    action_state: ActionPolicyState | None
    parent_logits: Tensor
    action_logits25: Tensor
    policy_logits26: Tensor
    max_action_logit: Tensor
    evidence_pool: Tensor
    direct_input: Tensor
    swap_logits: Tensor
    logits: Tensor
    swapped: Tensor
...
@dataclass(frozen=True)
class DirectSVRALoss:
    """Unweighted DESC loss components."""
...
class RoleToWindowActionPolicy(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        action_hidden = F.gelu(self.utility_hidden(action_head_input))
        utility_logits = self.utility_output(action_hidden).squeeze(2)
        abstain = torch.zeros(
...
class DirectSwapInteraction(nn.Module):
    """DESC I module: direct evidence-conditioned keep/swap competition."""

    def __init__(self) -> None:
        super().__init__()
        self.hidden = nn.Linear(DIRECT_SWAP_INPUT_DIM, HIDDEN_DIM)
        self.output = nn.Linear(HIDDEN_DIM, 1)
        nn.init.zeros_(self.output.weight)
        nn.init.zeros_(self.output.bias)

    def forward(self, direct_inputs: Tensor) -> Tensor:
        _require_rank("direct_inputs", direct_inputs, 2)
        _require_last_dim("direct_inputs", direct_inputs, DIRECT_SWAP_INPUT_DIM)
        return self.output(F.gelu(self.hidden(direct_inputs.float()))).squeeze(1)
...
class SemanticVisualRiskArbiter(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        with torch.random.fork_rng(devices=[]):
            ...
            self.direct_interaction = DirectSwapInteraction()
    ...
    def direct_forward(
        ...
        *,
        ...
        """Run the IDEA-202 / DESC direct keep-swap path."""

        return direct_svra_forward(
            self,
            full_cls,
            patch_tokens,
            semantic_off=semantic_off,
...
def direct_svra_forward(
    ...
    *,
    ...
    """Run DESC: Parent4 + max-action1 + pooled-evidence64 -> swap_logit."""

    if parent_only:
        pair = model.parent_state(full_cls, semantic_off=False)
        action_state = None
        action_logits25 = full_cls.new_zeros(full_cls.shape[0], ACTION_COUNT)
        policy_logits26 = full_cls.new_zeros(full_cls.shape[0], ACTION_COUNT + 1)
        max_action_logit = full_cls.new_zeros(full_cls.shape[0])
        evidence_pool = full_cls.new_zeros(full_cls.shape[0], HIDDEN_DIM)
    else:
        action_state = model.action_state(
            full_cls,
            patch_tokens,
        ...
        action_logits25 = action_state.utility_logits
        policy_logits26 = action_state.policy_logits
        max_action_logit = torch.amax(action_logits25, dim=1)
        weights = torch.softmax(action_logits25.float(), dim=1)
        evidence_pool = torch.einsum("ba,bad->bd", weights, action_state.action_hidden.float())

    direct_input = torch.cat(
        [
            pair.parent_stats.float(),
            max_action_logit.float().unsqueeze(1),
            evidence_pool.float(),
        ],
        dim=1,
    )
    if direct_input.shape[1] != DIRECT_SWAP_INPUT_DIM:
        raise RuntimeError(
            "DESC direct input contract violated: "
            f"expected {DIRECT_SWAP_INPUT_DIM}, got {direct_input.shape[1]}"
        )
    if interaction_off:
        swap_logits = full_cls.new_zeros(full_cls.shape[0])
        swapped = torch.zeros(full_cls.shape[0], dtype=torch.bool, device=full_cls.device)
        logits = pair.parent_logits
    else:
        swap_logits = model.direct_interaction(direct_input)
        swapped = swap_logits > 0
        logits, _, _ = ParentRiskArbiter.apply_keep_swap(
            pair.parent_logits,
            pair.top2,
            torch.ones_like(swapped),
            swap_logits,
        )
    return DirectSVRAOutput(
        pair=pair,
        action_state=action_state,
        parent_logits=pair.parent_logits,
        action_logits25=action_logits25,
        policy_logits26=policy_logits26,
        max_action_logit=max_action_logit,
...
def direct_svra_loss(
    output: DirectSVRAOutput,
    targets: JointSVRATargets,
    *,
    include_action: bool = True,
) -> DirectSVRALoss:
    """Compute DESC natural, unweighted BCE swap loss plus optional action CE."""

    if output.swap_logits.shape != targets.risk_targets.shape:
        raise ValueError(
            "swap_logits and y_swap shape mismatch: "
            f"{tuple(output.swap_logits.shape)} vs {tuple(targets.risk_targets.shape)}"
        )
    swap_target = targets.risk_targets.to(device=output.swap_logits.device).float()
    swap_loss = F.binary_cross_entropy_with_logits(
        output.swap_logits.float(),
        swap_target,
    )
    if include_action:
        ...
        action_loss = output.swap_logits.sum() * 0.0
    return DirectSVRALoss(
        total=swap_loss + action_loss,
        swap=swap_loss,
        action=action_loss,
        raw_means={"swap": swap_loss, "action": action_loss},
