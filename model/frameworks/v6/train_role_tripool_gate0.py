"""Gate-0 Full-only trainer for IDEA-197 / Role Tri-Pool.

The script is deliberately narrow:

* trains only the Full 25-window tri-state net-benefit objective
...
try:
    ...
    from role_tripool import (  # type: ignore[no-redef]
        ACTION_COUNT,
        ACTION_GEOMETRY_SHA256,
        FEATURE_DIM,
        PAIR_TEMPERATURE,
...
TRAIN_SCHEMA = "gzsl-paper.v6-role-tripool-gate0-train.v1"


TRIPOOL_PREREGISTERED_GROUP_COUNTS = {
    "total_rows": 4702,
    "leader_rows": 3478,
    "challenger_rows": 634,
    "outside_rows": 590,
...
def validate_config(config: Gate0TrainConfig) -> None:
    ...
    if config.condition_id != "ROLE_TRIPOOL_FULL":
        raise RWDGDataError("Gate0 training condition_id must be ROLE_TRIPOOL_FULL")
...
def tri_pool_action_targets(
    ...
    *,
    ...
    targets, groups, pair = model.signed_action_targets(
        full_cls,
        all_crop_cls,
        target_class_ids,
...
def _class_histogram(targets: Tensor) -> list[int]:
    return [
        int(x)
        for x in torch.bincount(
            targets.detach().cpu().long().reshape(-1), minlength=TRISTATE_CLASS_COUNT
        )[:TRISTATE_CLASS_COUNT].tolist()
    ]


def _group_target_counts(targets: Tensor, groups: Tensor) -> dict[str, dict[str, int]]:
    names = ("leader", "challenger", "outside")
    out: dict[str, dict[str, int]] = {}
    for group_id, name in enumerate(names):
        mask = groups == group_id
        group_targets = targets[mask]
        histogram = _class_histogram(group_targets)
        out[name] = {
            "rows": int(mask.sum().item()),
            "damage_actions": histogram[0],
            "neutral_actions": histogram[1],
            "correction_actions": histogram[2],
        }
    return out
...
def precompute_tri_pool_target_plan(
    model: RoleWindowDenseGlimpse,
    train_view: RWDGGateSubsetView,
    targets: TrainSubsetTargets,
    *,
    ...
    with torch.no_grad():
        for start in range(0, train_view.size, batch_size):
            ...
            batch_targets, groups, _ = tri_pool_action_targets(
                model,
                full_cls,
                crops,
                labels,
            )
            targets_cpu = batch_targets.detach().cpu().long()
            groups_cpu = groups.detach().cpu().long()
            if bool((targets_cpu < 0).any()) or bool((targets_cpu >= TRISTATE_CLASS_COUNT).any()):
                raise RWDGDataError("Role Tri-Pool targets must be class ids in [0,2]")
            all_groups.append(groups_cpu)
            all_targets.append(targets_cpu)

    groups = torch.cat(all_groups).long()
    all_action_targets = torch.cat(all_targets).long()
    group_counts = torch.bincount(groups, minlength=3)[:3].long()
    histogram = _class_histogram(all_action_targets)
    group_target_counts = _group_target_counts(all_action_targets, groups)
    ...
    stats = {
        ...
        "natural_row_sampling": True,
        "class_weights": None,
    }
    assert_pre_registered_tri_pool_counts(stats)
    return TriPoolTargetPlan(targets=all_action_targets, groups=groups, stats=stats)


def assert_pre_registered_tri_pool_counts(stats: Mapping[str, Any]) -> None:
    natural = stats.get("natural_distribution")
    if not isinstance(natural, Mapping):
        raise RWDGDataError("Role Tri-Pool target stats missing natural_distribution")
    groups = natural.get("group_counts")
    if not isinstance(groups, Mapping):
        raise RWDGDataError("Role Tri-Pool target stats missing group_counts")
    histogram = stats.get("class_histogram")
    if not isinstance(histogram, Sequence) or isinstance(histogram, (str, bytes)):
        raise RWDGDataError("Role Tri-Pool target stats missing class_histogram")
    ...
    mismatches = {
        ...
        for key, expected in TRIPOOL_PREREGISTERED_GROUP_COUNTS.items()
        if actual[key] != expected
    }
    if len(histogram) != TRISTATE_CLASS_COUNT or any(int(x) <= 0 for x in histogram):
        mismatches["tri_state_histogram"] = {
            "actual": [int(x) for x in histogram],
            "expected": "all damage/neutral/correction classes present",
        }
    if mismatches:
        raise RWDGDataError(f"Role Tri-Pool target counts mismatch: {mismatches}")


class NaturalRowSampler:
    """Seeded uniform row sampler that preserves the natural train distribution."""
    ...
    def __init__(
        ...
        *,
        ...
        if groups.numel() < batch_size:
            raise RWDGDataError("Role Tri-Pool sampler has fewer rows than one batch")
        self.batch_size = int(batch_size)
        self.seed = int(seed)
        self.row_count = int(groups.numel())
        self.group_counts = torch.bincount(groups, minlength=3)[:3].long()
    ...
    def sample(self) -> Tensor:
        ...
        while remaining > 0:
            if self._buffer.numel() == 0:
                self._buffer = torch.randperm(self.row_count, generator=self._generator)
            take = min(remaining, int(self._buffer.numel()))
    ...
    def state_dict(self) -> Mapping[str, Any]:
        return {
            "name": "NaturalRowSampler",
            "seed": self.seed,
            "batch_size": self.batch_size,
            "row_count": self.row_count,
            "group_counts": [int(x) for x in self.group_counts.tolist()],
            "sampling_distribution": "natural_uniform_rows",
            "reshuffle_policy": "seeded randperm cycles over all rows",
        }
...
def sampled_class_histogram(
    ...
    *,
    ...
    sampler = NaturalRowSampler(
        groups,
        batch_size=batch_size,
        seed=seed,
    )
    histogram = torch.zeros(TRISTATE_CLASS_COUNT, dtype=torch.long)
    for _ in range(int(updates)):
        rows = sampler.sample()
        batch_targets = targets.index_select(0, rows).long().reshape(-1)
        histogram += torch.bincount(
            batch_targets.cpu(), minlength=TRISTATE_CLASS_COUNT
        )[:TRISTATE_CLASS_COUNT].long()
    if bool((histogram <= 0).any()):
        raise RWDGDataError(
            f"Role Tri-Pool sampled trace must include all three states: {histogram.tolist()}"
        )
    total_actions = max(int(updates) * int(batch_size) * ACTION_COUNT, 1)
    return {
        "updates": int(updates),
        "batch_size": int(batch_size),
        "sampled_rows": int(updates) * int(batch_size),
        "sampled_actions": total_actions,
        "class_histogram": [int(x) for x in histogram.tolist()],
        "class_names": ["damage", "neutral", "correction"],
        "class_density": [float(x / total_actions) for x in histogram.tolist()],
...
def train_step(
    ...
    *,
    ...
    batch_targets, _, _ = tri_pool_action_targets(model, full_cls, crops, labels)
    utility = model.utility_state(full_cls, patches)
    loss = tri_state_action_loss(utility.utility_logits, batch_targets)
    loss.backward()
...
def collect_gradient_report(model: RoleWindowDenseGlimpse) -> Mapping[str, GradientGateReport]:
    named = {
        "input_norm": model.visual.input_norm.weight,
...
def static_best_action_from_natural_targets(targets: Tensor) -> tuple[int, Mapping[str, Any]]:
    ...
    if targets.ndim != 2 or targets.shape[1] != ACTION_COUNT:
        raise RWDGDataError("natural Role Tri-Pool targets must have shape [N,25]")
    if bool((targets < 0).any()) or bool((targets >= TRISTATE_CLASS_COUNT).any()):
        raise RWDGDataError("natural Role Tri-Pool targets must be class ids in [0,2]")
    corrections = targets.eq(2).sum(dim=0).long()
    damages = targets.eq(0).sum(dim=0).long()
    net = corrections - damages
    action = int(torch.argmax(net).item())  # smallest ID breaks ties.
    return action, {
    ...
    *,
    ...
    with torch.no_grad():
        for start in range(0, train_view.size, batch_size):
            rows = torch.arange(start, min(start + batch_size, train_view.size), dtype=torch.long)
            full_cls, _, crops, labels = batch_from_rows(train_view, targets, rows, device=device)
            batch_targets, groups, _ = tri_pool_action_targets(model, full_cls, crops, labels)
            group_counts += torch.bincount(groups.detach().cpu(), minlength=3)[:3]
            all_targets.append(batch_targets.detach().cpu().long())
    targets_all = torch.cat(all_targets).long()
    static_best, detail = static_best_action_from_natural_targets(targets_all)
    return {
...
def train_full_gate0(
    ...
    *,
    ...
    try:
        ...
        target_plan = precompute_tri_pool_target_plan(
            model,
            train_view,
            train_targets,
        ...
        static_best_action, static_best_detail = static_best_action_from_natural_targets(target_plan.targets)
        static_best = {
            ...
            **dict(static_best_detail),
            ...
            "tri_state_class_histogram": target_plan.stats["class_histogram"],
        }
        ...
        checkpoint_path = output_dir / "role_tripool_gate0_full.pt"
...
__all__ = [
    ...
    "prepare_output_dir",
    "precompute_tri_pool_target_plan",
