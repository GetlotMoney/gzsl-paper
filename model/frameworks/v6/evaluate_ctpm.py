...
def evaluate(
    ...
    *,
    ...
    variants = {
        ...
        "semantic_only": {"enable_v": False, "enable_i": False},
    ...
    vi_pair_equal = all(
        torch.equal(pairs["full"][split], pairs[name][split])
        for name in ("V_off", "I_off", "semantic_only")
        for split in ("seen", "unseen", "zs")
    )
    parent_pair_equal = all(
        torch.equal(pairs["parent"][split], pairs[name][split])
        for name in ("parent", "S_off")
        for split in ("seen", "unseen", "zs")
    )
    if not vi_pair_equal or not parent_pair_equal:
        raise RuntimeError("CTPM candidate pair changed outside the semantic prototype boundary.")
    stratified = {}
    for split, labels in (
    ...
    return {
        **full,
        ...
        "semantic_parent_metrics": scores["semantic_only"],
        ...
        "controls": {"S_query_off": scores["S_query_off"]},
        "pair_identity_equal_across_offs": vi_pair_equal and parent_pair_equal,
        "pair_identity_contract": {
            "full_equals_V_off_I_off": vi_pair_equal,
            "parent_equals_S_off": parent_pair_equal,
        },
