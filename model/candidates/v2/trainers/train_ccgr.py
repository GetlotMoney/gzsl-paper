...
from model.frameworks.v4.ccgr import (
    ClassConditionedGeometricGenerator,
    NeighborhoodResidualCCGR,
...
from model.candidates.v2.trainers.train_elpt import FrozenPrototypeClassifier, _candidate_prototypes, _fold_package, _load_fold_checkpoint
...
CONFIG_KEYS_NEIGHBOR=CONFIG_KEYS_EPOCH|{"class_feature_mode"}
CONFIG_KEYS_NEIGHBOR_RESIDUAL=CONFIG_KEYS_NEIGHBOR|{"parent_ccgr_model","parent_ccgr_model_sha256"}
CONFIG_KEYS_HARMONIC=CONFIG_KEYS_NEIGHBOR_RESIDUAL|{"harmonic_weight"}
CONFIG_KEYS_LOCAL_BOUNDARY=CONFIG_KEYS_HARMONIC|{"hard_negative_weight","hard_negative_margin"}
...
def load_config(path:Path):
    ...
    expected=CONFIG_KEYS_LOCAL_BOUNDARY if schema=="gzsl-paper.ccgr-lbs.v1" else (CONFIG_KEYS_HARMONIC if schema=="gzsl-paper.ccgr-heo.v1" else (CONFIG_KEYS_NEIGHBOR_RESIDUAL if schema=="gzsl-paper.ng-ccgr-residual.v1" else (CONFIG_KEYS_NEIGHBOR if schema=="gzsl-paper.ng-ccgr.v1" else (CONFIG_KEYS_EPOCH if schema=="gzsl-paper.ccgr-epoch-select.v1" else (CONFIG_KEYS_MARGIN if schema=="gzsl-paper.ccgr-margin.v1" else (CONFIG_KEYS_V4 if schema=="gzsl-paper.ccgr.v4" else (CONFIG_KEYS_V3 if schema in ("gzsl-paper.ccgr.v3","gzsl-paper.ccgr.tune.v1") else (CONFIG_KEYS_V2 if schema=="gzsl-paper.ccgr.v2" else CONFIG_KEYS))))))))
    ...
    valid_ids=("V2-TRY-058","V2-TRY-059","V2-TRY-060","V2-TRY-061","V2-TRY-062","V2-TRY-063","V2-TRY-064","V2-TRY-065","V2-TRY-066","V2-TRY-067","V2-TRY-074","V2-TRY-077","V2-TRY-078","V2-TRY-079","V2-TRY-080","V2-TRY-081","V2-TRY-082","V2-TRY-083","V2-TRY-084","V2-TRY-085")
    expected_idea="IDEA-027" if config.get("attempt_id")=="V2-TRY-085" else ("IDEA-026" if config.get("attempt_id") in ("V2-TRY-083","V2-TRY-084") else ("IDEA-025" if config.get("attempt_id") in ("V2-TRY-081","V2-TRY-082") else ("IDEA-022" if config.get("attempt_id")=="V2-TRY-074" else "IDEA-018")))
    if config["schema_version"] not in ("gzsl-paper.ccgr.v1","gzsl-paper.ccgr.v2","gzsl-paper.ccgr.v3","gzsl-paper.ccgr.v4","gzsl-paper.ccgr.tune.v1","gzsl-paper.ccgr-margin.v1","gzsl-paper.ccgr-epoch-select.v1","gzsl-paper.ng-ccgr.v1","gzsl-paper.ng-ccgr-residual.v1","gzsl-paper.ccgr-heo.v1","gzsl-paper.ccgr-lbs.v1") or config["attempt_id"] not in valid_ids or config["idea_id"]!=expected_idea: raise ValueError("CCGR首次TRY身份错误。")
    ...
    expected_max={"V2-TRY-066":0.15,"V2-TRY-067":0.2,"V2-TRY-074":0.2,"V2-TRY-077":0.2,"V2-TRY-078":0.2,"V2-TRY-079":0.2,"V2-TRY-080":0.2,"V2-TRY-081":0.2,"V2-TRY-082":0.2,"V2-TRY-083":0.2,"V2-TRY-084":0.2,"V2-TRY-085":0.2}.get(config["attempt_id"],0.1)
    ...
    config.setdefault("class_feature_mode","top5_mean")
    config.setdefault("harmonic_weight",0.0)
    config.setdefault("hard_negative_weight",0.0)
    config.setdefault("hard_negative_margin",0.0)
    ...
    if config["attempt_id"] in ("V2-TRY-060","V2-TRY-062","V2-TRY-063","V2-TRY-064","V2-TRY-065","V2-TRY-066","V2-TRY-067","V2-TRY-074","V2-TRY-077","V2-TRY-078","V2-TRY-079","V2-TRY-080","V2-TRY-081","V2-TRY-082","V2-TRY-083","V2-TRY-084","V2-TRY-085") and (not config["fold_checkpoint_dir"] or int(config["batch_half"])!=32 or float(config["pseudo_unseen_weight"])!=0.25): raise ValueError("CCGR unseen风险配置错误。")
    ...
    if config["attempt_id"] in ("V2-TRY-077","V2-TRY-078","V2-TRY-079","V2-TRY-080","V2-TRY-081","V2-TRY-082","V2-TRY-083","V2-TRY-084","V2-TRY-085") and config["select_each_epoch"] is not True: raise ValueError("CCGR逐epoch选择配置错误。")
    if config["attempt_id"] in ("V2-TRY-081","V2-TRY-082") and config["class_feature_mode"]!="top5_vector": raise ValueError("NG-CCGR必须使用完整top-5类别邻域。")
    if config["attempt_id"]=="V2-TRY-082" and (not config["parent_ccgr_model"] or not config["parent_ccgr_model_sha256"]): raise ValueError("邻域残差补救必须绑定父CCGR模型。")
    if config["attempt_id"]=="V2-TRY-083" and (config["class_feature_mode"]!="top5_mean" or not config["parent_ccgr_model"] or float(config["harmonic_weight"])!=1.0): raise ValueError("CCGR-HEO首次TRY配置错误。")
    if config["attempt_id"]=="V2-TRY-084" and (config["class_feature_mode"]!="top5_mean" or not config["parent_ccgr_model"] or float(config["harmonic_weight"])!=0.1): raise ValueError("CCGR-HEO保守补救配置错误。")
    if config["attempt_id"]=="V2-TRY-085" and (config["class_feature_mode"]!="top5_mean" or not config["parent_ccgr_model"] or float(config["hard_negative_weight"])!=0.1 or float(config["hard_negative_margin"])!=0.02): raise ValueError("CCGR-LBS首次TRY配置错误。")
    return config,sha256_file(path)


def ccgr_class_features(base,value,text_resultant,top5,mode):
    common=torch.stack(((base*value).sum(dim=-1),(value-base).norm(dim=-1),text_resultant),dim=1)
    if mode=="top5_mean": return torch.cat((common,top5.mean(dim=1,keepdim=True)),dim=1)
    if mode=="top5_vector": return torch.cat((common,top5),dim=1)
    raise ValueError(f"未知CCGR类别特征模式: {mode}")


def harmonic_episode_loss(logits,targets,batch_half):
    probabilities=torch.softmax(logits,dim=-1)
    correct=probabilities.gather(1,targets.unsqueeze(1)).squeeze(1)
    seen_score=correct[:batch_half].mean()
    unseen_score=correct[batch_half:].mean()
    harmonic=2.0*seen_score*unseen_score/(seen_score+unseen_score).clamp_min(1e-8)
    return 1.0-harmonic,seen_score,unseen_score


def local_boundary_loss(generated,visual_centroids,class_ids,margin):
    prototypes=torch.nn.functional.normalize(generated.index_select(0,class_ids),dim=-1)
    centroids=torch.nn.functional.normalize(visual_centroids,dim=-1)
    scores=centroids@prototypes.T
    positive=scores.diagonal()
    negative=scores.masked_fill(torch.eye(scores.shape[0],device=scores.device,dtype=torch.bool),float("-inf")).max(dim=1).values
    return torch.relu(float(margin)+negative-positive).mean()
...
def run(config_path:Path,output_dir:Path,expected_commit:str):
    ...
    parent_ccgr_path=Path(config["parent_ccgr_model"]) if config.get("parent_ccgr_model") else None
    if parent_ccgr_path is not None and sha256_file(parent_ccgr_path)!=config["parent_ccgr_model_sha256"]: raise ValueError("NG-CCGR父模型SHA不匹配。")
    ...
    try:
        ...
        if config["training_objective"]=="pseudo_unseen_episode":
            for fold_id,(ps,pu) in enumerate(folds):
                ...
                with torch.no_grad():
                    fold_base=package["base_all"].to(device); fold_full=package["fold_full"].to(device).clone(); fold_value_all=fold_model.value_candidate(allclasses.to(device)); fold_roles=fold_model.semantic_group_vectors(); fold_basis=tangent_direction_basis(fold_base,fold_value_all,fold_roles); support=fold_base.index_select(0,ps.to(device)); fold_top5=(fold_base@support.T).topk(5,dim=1).values; fold_features=ccgr_class_features(fold_base,fold_value_all,text_resultant,fold_top5,config["class_feature_mode"]); step=ntr_gate(package["gate_features"].to(device)); pu_device=pu.to(device); fold_full[pu_device]=tangent_transport(fold_base.index_select(0,pu_device),package["value"].to(device),step); package["pseudo_unseen_centroids"]=centroids.index_select(0,torch.searchsorted(seenclasses,pu)).to(device)
        ...
        if official_loaded:
            ...
            if parent_ccgr_path is not None:
                initial_official=h1.evaluate(model,tensors,seenclasses,unseenclasses,device); best_H=initial_official["H"]; best_epoch=0; best_state=copy.deepcopy(model.state_dict()); history.append({"epoch":0,"official_metrics_percent":initial_official,"magnitude":model.magnitude_stats()}); print(f"epoch=0 official_H={best_H:.6f} magnitude={model.magnitude_stats()}")
        for epoch in range(1,int(config["epochs"])+1):
            if config["training_objective"]=="pseudo_unseen_episode":
                loss_sum=0.0; harmonic_sum=0.0; boundary_sum=0.0; count=0
