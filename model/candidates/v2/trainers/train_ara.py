...
CONFIG_KEYS_V4=CONFIG_KEYS_V2|{"ridge_source"}
...
def load_config(path):
    ...
    if config["schema_version"] not in ("gzsl-paper.ara.v1","gzsl-paper.ara.v2","gzsl-paper.ara.v3","gzsl-paper.ara.v4") or config["attempt_id"] not in ("V2-TRY-092","V2-TRY-093","V2-TRY-094","V2-TRY-095","V2-TRY-096","V2-TRY-097","V2-TRY-098","V2-TRY-099","V2-TRY-100","V2-TRY-104","V2-TRY-105","V2-TRY-106","V2-TRY-107","V2-TRY-109","V2-TRY-110","V2-TRY-111","V2-TRY-112") or config["idea_id"] not in ("IDEA-029","IDEA-033"): raise ValueError("ARA首次TRY身份错误。")
    expected_ridge={"V2-TRY-109":0.1,"V2-TRY-110":1.0}.get(config["attempt_id"],0.01)
    if int(config["epochs"])!=20 or int(config["batch_size"])!=256 or float(config["lr"])!=0.01 or float(config["weight_decay"])!=0.0 or float(config["ridge"])!=expected_ridge or float(config["max_beta"])!=20.0: raise ValueError("ARA训练参数错误。")
    ...
    if config["attempt_id"] in ("V2-TRY-104","V2-TRY-105","V2-TRY-106","V2-TRY-107","V2-TRY-109","V2-TRY-110","V2-TRY-111","V2-TRY-112") and (config["ridge_source"]!="class_centroids" or config["sdm_enabled"] is not False or config["idea_id"]!="IDEA-033"): raise ValueError("CRA类别中心ridge配置错误。")
...
def run(config_path,output_dir,expected_commit,resume_from=None):
    ...
    try:
        ...
        for epoch in range(start_epoch,int(config["epochs"])+1):
            ...
            atomic_torch_save(output_dir/"checkpoint_last.pth",{"schema_version":"gzsl-paper.cra-checkpoint.v1","attempt_id":config["attempt_id"],"code_commit":code_commit,"config_sha256":config_sha,"epoch":epoch,"ara_state_dict":copy.deepcopy(ara.state_dict()),"optimizer_state_dict":copy.deepcopy(optimizer.state_dict()),"generator_state":generator.get_state(),"history":history,"best_H":best_H,"best_epoch":best_epoch,"best_state_dict":best_state})
        last_state=copy.deepcopy(ara.state_dict()); ara.load_state_dict(best_state,strict=True); best_payload={"attempt_id":config["attempt_id"],"code_commit":code_commit,"config":config,"selected_epoch":best_epoch,"ara_state_dict":best_state,"history":history}; atomic_torch_save(output_dir/"ara_model.pth",best_payload); shutil.copyfile(output_dir/"ara_model.pth",output_dir/"model_best.pth"); parent_ara=AttributeResidualAlignment(ridge_weight,attributes,config["max_beta"]).to(device); parent_metrics=evaluate(prototypes,ccgr.scale(),metric,parent_ara,tensors,seenclasses,unseenclasses,device); candidate_metrics=evaluate(prototypes,ccgr.scale(),metric,ara,tensors,seenclasses,unseenclasses,device); delta={key:candidate_metrics[key]-float(config["parent_metrics_percent"][key]) for key in ("U","S","H","ZS")}; beta=float(ara.beta().detach()); threshold=79.44820966336283 if config["attempt_id"] in ("V2-TRY-109","V2-TRY-110") else (79.38608193220331 if config["attempt_id"]=="V2-TRY-104" else 78.0); success=candidate_metrics["H"]>threshold and delta["U"]>=-2 and delta["S"]>=-2 and abs(beta)<19.6; atomic_write_json(output_dir/"data_fingerprints.json",{"files":input_sha}); metrics={"attempt_id":config["attempt_id"],"idea_id":config["idea_id"],"framework_id":config["framework_id"],"code_commit":code_commit,"config_sha256":config_sha,"base_config_sha256":base_config_sha,"evaluation_protocol":h1.EVALUATION_PROTOCOL,"test_used_for_selection":True,"unseen_images_used_for_gradient":False,"selected_epoch":best_epoch,"ridge":float(config["ridge"]),"ridge_source":config["ridge_source"],"learned_beta":beta,"sdm_enabled":bool(config["sdm_enabled"]),"ccgr_enabled":bool(config["ccgr_enabled"]),"resumed_from":str(Path(resume_from).resolve()) if resume_from is not None else None,"resumed_checkpoint_sha256":resume_sha,"recomputed_parent_metrics_percent":parent_metrics,"parent_metrics_percent":config["parent_metrics_percent"],"candidate_metrics_percent":candidate_metrics,"delta_vs_parent_percent_points":delta,"success":success,"ara_model_sha256":sha256_file(output_dir/"ara_model.pth"),"model_best_sha256":sha256_file(output_dir/"model_best.pth"),"checkpoint_last_sha256":sha256_file(output_dir/"checkpoint_last.pth")}; atomic_write_json(output_dir/"metrics.json",metrics); print(metrics); return metrics
    finally:
        sys.stdout.flush(); sys.stdout=original_stdout; log_handle.close()


def main():
    parser=argparse.ArgumentParser(description=__doc__); parser.add_argument("--config",type=Path,required=True); parser.add_argument("--output-dir",type=Path,required=True); parser.add_argument("--expected-commit",required=True); parser.add_argument("--resume-from",type=Path); args=parser.parse_args(); run(args.config,args.output_dir,args.expected_commit,args.resume_from)
