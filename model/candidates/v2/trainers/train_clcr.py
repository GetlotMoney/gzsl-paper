...
CONFIG_KEYS = {
    ...
    "claude_embeddings", "claude_embeddings_sha256", "max_beta", "device",
...
def load_config(path: Path):
    ...
    if not isinstance(config, dict) or actual != CONFIG_KEYS:
        raise ValueError(
            f"CLCR配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，"
            f"多出={sorted(actual-CONFIG_KEYS)}。"
        )
    if (
        config["schema_version"] != "gzsl-paper.clcr.v1"
        or config["experiment_id"] != "V2-INNOVATION-051"
        or config["idea_id"] != "IDEA-085"
    ):
        raise ValueError("CLCR身份错误。")
    if (
        ...
        raise ValueError("CLCR协议边界错误。")
    if config["text_cache_provenance_complete"] is not False:
        raise ValueError("CLCR文本cache provenance未完整。")
    if (
        float(config["max_beta"]) != 5.0
...
def run(config_path: Path, output_dir: Path, expected_commit: str, run_id: str):
    ...
    for key in (
        ...
        if sha256_file(Path(config[key])) != config[f"{key}_sha256"]:
            raise ValueError(f"CLCR {key} SHA错误。")
    ...
    try:
        ...
        if not torch.equal(checked_seen, seen_classes) or not torch.equal(
            ...
            raise ValueError("CLCR CUB类别边界错误。")
        ...
        if oclr_payload["config"]["schema_version"] != "gzsl-paper.oclr.v1":
            raise ValueError("CLCR绑定的父模型不是OCLR。")
        names_n = F.normalize(class_names.float(), dim=-1)
        ...
        model = CrossLLMComplementaryResidual(
            sdcr_prototypes, fixed_beta, claude_orth, float(config["max_beta"])
        ).to(device)
        ...
        for key in ("U", "S", "H", "ZS"):
            if abs(best_metrics[key] - float(expected_parent[key])) > 1e-5:
                raise ValueError(f"CLCR初始态未复现SDCR：{key}。")
        ...
        for iteration in range(int(config["niters"])):
            ...
            if iteration % int(config["report_interval"]) == 0:
                ...
                print(
                    ...
                    f"best_H={best_h:.6f} beta={stats['claude_beta']:.6f}"
                )

        atomic_torch_save(
            output_dir / "checkpoint_last.pth",
            {
                "clcr_state_dict": copy.deepcopy(model.state_dict()),
