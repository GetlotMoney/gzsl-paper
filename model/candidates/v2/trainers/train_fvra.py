...
from model.candidates.v2.modules.fvra import FeatureAdapterClassifier, FeatureVisualResidualAdapter
from model.candidates.v2.trainers.train_elpt import _candidate_prototypes
...
CONFIG_KEYS_V2=CONFIG_KEYS|{"max_residual_norm"}
...
def load_config(path:Path):
    ...
    expected=CONFIG_KEYS_V2 if isinstance(config,dict) and config.get("schema_version")=="gzsl-paper.fvra.v2" else CONFIG_KEYS
    if not isinstance(config,dict) or actual!=expected: raise ValueError(f"FVRA配置字段错误；缺少={sorted(expected-actual)}，多出={sorted(actual-expected)}。")
    if config["schema_version"] not in ("gzsl-paper.fvra.v1","gzsl-paper.fvra.v2") or config["attempt_id"] not in ("V2-TRY-068","V2-TRY-069") or config["idea_id"]!="IDEA-019": raise ValueError("FVRA首次TRY身份错误。")
    ...
    expected_consistency=0.1 if config["attempt_id"]=="V2-TRY-068" else 1.0
    if int(config["hidden_dim"])!=64 or float(config["residual_scale"])!=0.1 or float(config["consistency_weight"])!=expected_consistency: raise ValueError("FVRA模块参数错误。")
    if set(config["parent_metrics_percent"])!={"U","S","H","ZS"}: raise ValueError("FVRA父指标不完整。")
    config.setdefault("max_residual_norm",None)
    expected_bound=None if config["attempt_id"]=="V2-TRY-068" else 0.1
    if config["max_residual_norm"]!=expected_bound: raise ValueError("FVRA残差边界错误。")
