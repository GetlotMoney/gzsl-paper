...
CONFIG_KEYS = {
    ...
    "eight_sentence_embeddings_sha256", "group_rule", "max_beta", "device",
...
def run(config_path: Path, output_dir: Path, expected_commit: str, run_id: str):
    ...
    for key in (
        ...
        if sha256_file(Path(config[key])) != config[f"{key}_sha256"]:
            raise ValueError(f"TIGR {key} SHA错误。")
    ...
    try:
        ...
        if not torch.equal(checked_seen, seen_classes) or not torch.equal(
            ...
            raise ValueError("TIGR CUB类别边界错误。")
        ...
        for key in ("U", "S", "H", "ZS"):
            if abs(best_metrics[key] - float(expected_parent[key])) > 1e-5:
                raise ValueError(f"TIGR初始态未复现SDCR：{key}。")
        ...
        atomic_torch_save(
            ...
            {
                "tigr_state_dict": copy.deepcopy(model.state_dict()),
