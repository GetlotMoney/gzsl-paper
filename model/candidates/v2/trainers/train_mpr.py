...
from model.candidates.v2.modules.mpr import MultiRolePrototypeClassifier
...
CONFIG_KEYS_V2 = CONFIG_KEYS | {"learn_role_bias","max_role_bias"}
...
def load_config(path: Path):
    ...
    expected=CONFIG_KEYS_V2 if isinstance(config,dict) and config.get("schema_version")=="gzsl-paper.mpr.v2" else CONFIG_KEYS
    if not isinstance(config,dict) or actual!=expected: raise ValueError(f"MPR配置字段错误；缺少={sorted(expected-actual)}，多出={sorted(actual-expected)}。")
    if config["schema_version"] not in ("gzsl-paper.mpr.v1","gzsl-paper.mpr.v2") or config["attempt_id"] not in ("V2-TRY-046","V2-TRY-047") or config["idea_id"]!="IDEA-014": raise ValueError("MPR首次TRY身份错误。")
    ...
    config.setdefault("learn_role_bias",False); config.setdefault("max_role_bias",0.05)
    if config["learn_role_bias"] is not (config["attempt_id"]=="V2-TRY-047") or float(config["max_role_bias"])!=0.05: raise ValueError("MPR角色偏置身份错误。")
...
def run(config_path:Path,output_dir:Path,expected_commit:str):
    ...
    try:
        ...
        for epoch in range(1,int(config["epochs"])+1):
            ...
            row={"epoch":epoch,"ce":loss_sum/count,"strength":float(model.strength().detach()),"role_bias":model.role_bias().detach().tolist()}; history.append(row); print(f"epoch={epoch} ce={row['ce']:.6f} strength={row['strength']:.6f} role_bias={row['role_bias']}")
