...
CONFIG_KEYS = {
    ...
    "eight_sentence_embeddings", "eight_sentence_embeddings_sha256", "hidden_dim",
    "max_logit_residual", "kl_weight", "eval_batch_size", "diagnostic_count",
...
def load_config(path: Path):
    ...
    if not isinstance(config, dict) or actual != CONFIG_KEYS:
        raise ValueError(
            f"ICSR配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，"
            f"多出={sorted(actual-CONFIG_KEYS)}。"
        )
    if (
        config["schema_version"] != "gzsl-paper.icsr.v1"
        or config["experiment_id"] != "V2-INNOVATION-040"
        or config["idea_id"] != "IDEA-074"
    ):
        raise ValueError("ICSR身份错误。")
    if (
        ...
        raise ValueError("ICSR协议边界错误。")
    if config["text_cache_provenance_complete"] is not False:
        raise ValueError("ICSR文本cache provenance未完整。")
    if (
        int(config["hidden_dim"]) != 32
        or float(config["max_logit_residual"]) != 0.5
        or float(config["kl_weight"]) != 0.01
        or int(config["eval_batch_size"]) != 128
        or int(config["diagnostic_count"]) != 512
    ):
        raise ValueError("ICSR门控或评估参数错误。")
    if (
        ...
        raise ValueError("ICSR训练参数错误。")
    return config, sha256_file(path)


@torch.no_grad()
def evaluate(
    parent, sdrs, calibrator, model, tensors,
    seen_classes, unseen_classes, device, batch_size
):
    ...
    def predict(features, class_ids=None):
        ...
        for start in range(0, features.shape[0], int(batch_size)):
            ...
            outputs.append(predictions if class_ids is None else class_ids[predictions])
...
def run(config_path: Path, output_dir: Path, expected_commit: str, run_id: str):
    ...
    for key in (
        ...
        if sha256_file(Path(config[key])) != config[f"{key}_sha256"]:
            raise ValueError(f"ICSR {key} SHA错误。")
    ...
    try:
        ...
        if not torch.equal(checked_seen, seen_classes) or not torch.equal(checked_unseen, unseen_classes):
            raise ValueError("ICSR CUB类别边界错误。")
        ...
        model = ImageConditionedSentenceRouting(
            new_sentences, class_names, base_weights, fixed_beta,
            int(config["hidden_dim"]), float(config["max_logit_residual"]),
        ).to(device)
        ...
        diagnostic_features = features[: int(config["diagnostic_count"])].to(device).float()

        best_metrics = evaluate(
            parent, sdrs, calibrator, model, official,
            seen_classes, unseen_classes, device, int(config["eval_batch_size"]),
        )
        expected_parent = config["parent_metrics_percent"]
        for key in ("U", "S", "H", "ZS"):
            if abs(best_metrics[key] - float(expected_parent[key])) > 1e-5:
                raise ValueError(f"ICSR初始态未复现CASR：{key}。")
        ...
        for iteration in range(int(config["niters"])):
            ...
            kl_loss = model.kl_to_base(images)
            ...
            if iteration % int(config["report_interval"]) == 0:
                ...
                stats = model.routing_stats(diagnostic_features)
                history.append(
                    {
                        "iteration": iteration,
                ...
                print(
                    ...
                    f"best_H={best_h:.6f} image_var={stats['image_variation']:.6f}"
                )

        atomic_torch_save(
            output_dir / "checkpoint_last.pth",
            {
                "icsr_state_dict": copy.deepcopy(model.state_dict()),
        ...
        best_stats = model.routing_stats(diagnostic_features)
        atomic_write_json(
