...
def load_config(path):
    ...
    if config["schema_version"]!="gzsl-paper.vebc.v1" or config["attempt_id"] not in ("V2-TRY-119","V2-TRY-120","V2-TRY-121","V2-TRY-125","V2-TRY-126","V2-TRY-127","V2-TRY-130") or config["idea_id"]!="IDEA-037": raise ValueError("VEBC首次TRY身份错误。")
    expected_lr=0.0025 if config["attempt_id"] in ("V2-TRY-120","V2-TRY-121","V2-TRY-125","V2-TRY-126","V2-TRY-127","V2-TRY-130") else 0.01
    expected_gamma=0.3 if config["attempt_id"] in ("V2-TRY-121","V2-TRY-125","V2-TRY-126","V2-TRY-127","V2-TRY-130") else 0.25
    expected_reverse=0.1 if config["attempt_id"]=="V2-TRY-130" else 0.01
    if int(config["epochs"])!=20 or int(config["batch_half"])!=32 or float(config["lr"])!=expected_lr or float(config["weight_decay"])!=0.0 or float(config["forward_ridge"])!=0.01 or float(config["reverse_ridge"])!=expected_reverse or float(config["max_beta"])!=20.0 or float(config["max_vpa_beta"])!=20.0 or float(config["max_gamma"])!=expected_gamma: raise ValueError("VEBC训练参数错误。")
    return config,sha256_file(path)


def _load_gate(config,device):
    path=Path(config["ntr_gate_model"])
    if sha256_file(path)!=config["ntr_gate_model_sha256"]: raise ValueError("VEBC父NTR gate SHA不匹配。")
...
@torch.no_grad()
def evaluate(prototypes,scale,vpa,calibrator,tensors,seenclasses,unseenclasses,device):
    ...
    def full_predict(features):
        logits=vpa.logits(features.to(device).float(),prototypes,scale,identity); return calibrator(logits,seen_mask).argmax(dim=1).cpu()
    seen_pred=full_predict(tensors["seen_features"]); unseen_pred=full_predict(tensors["unseen_features"]); zsl_logits=vpa.logits(tensors["unseen_features"].to(device).float(),prototypes,scale,identity,unseenclasses); zsl_pred=unseenclasses[zsl_logits.argmax(dim=1).cpu()]; seen=h1.per_class_accuracy(tensors["seen_labels"],seen_pred,seenclasses); unseen=h1.per_class_accuracy(tensors["unseen_labels"],unseen_pred,unseenclasses); zsl=h1.per_class_accuracy(tensors["unseen_labels"],zsl_pred,unseenclasses); harmonic=2*seen*unseen/(seen+unseen) if seen+unseen else 0.0; return {"U":unseen*100,"S":seen*100,"H":harmonic*100,"ZS":zsl*100}
...
def run(config_path,output_dir,expected_commit):
    ...
    base_config,base_config_sha=h1.load_config(base_path); paths=h1.resolve_paths(base_config); input_sha=h1.verify_inputs(base_config,paths,h1.TRAINING_KEYS); checkpoint_path=Path(config["base_checkpoint"]); ccgr_path=Path(config["ccgr_model"]); cra_path=Path(config["cra_model"]); vpa_path=Path(config["vpa_model"])
    if sha256_file(checkpoint_path)!=config["base_checkpoint_sha256"] or sha256_file(ccgr_path)!=config["ccgr_model_sha256"] or sha256_file(cra_path)!=config["cra_model_sha256"] or sha256_file(vpa_path)!=config["vpa_model_sha256"]: raise ValueError("VEBC父模型SHA不匹配。")
    attribute_sha=sha256_file(paths["att_splits"])
    if attribute_sha!=base_config["expected_sha256"]["att_splits"]: raise ValueError("VEBC属性文件SHA不匹配。")
    input_sha["att_splits"]=attribute_sha; device=torch.device(base_config["device"])
    if device.type!="cuda" or not torch.cuda.is_available(): raise RuntimeError("VEBC要求CUDA。")
