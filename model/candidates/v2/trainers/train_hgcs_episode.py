...
from model.candidates.v2.modules.hgcs import HierarchicalGroupCommonSuppression
...
from model.candidates.v2.modules.vpa import fit_attribute_to_visual_map
...
def load_config(path):
    ...
    if not isinstance(config,dict) or actual!=CONFIG_KEYS: raise ValueError(f"HGCS episode配置字段错误；缺少={sorted(CONFIG_KEYS-actual)}，多出={sorted(actual-CONFIG_KEYS)}。")
    if config["schema_version"]!="gzsl-paper.hgcs-episode.v1" or config["attempt_id"]!="V2-TRY-147" or config["idea_id"]!="IDEA-043": raise ValueError("HGCS episode补救身份错误。")
    if int(config["epochs"])!=20 or int(config["batch_half"])!=32 or float(config["lr"])!=0.01 or float(config["weight_decay"])!=0.0 or float(config["forward_ridge"])!=0.01 or float(config["reverse_ridge"])!=0.01 or float(config["max_beta"])!=20.0 or float(config["max_vpa_beta"])!=20.0 or float(config["max_gamma"])!=0.3 or float(config["max_name_beta"])!=5.0 or int(config["group_count"])!=20 or float(config["max_group_beta"])!=10.0: raise ValueError("HGCS episode参数错误。")
    return config,sha256_file(path)
def _load_gate(config,device):
    path=Path(config["ntr_gate_model"])
    if sha256_file(path)!=config["ntr_gate_model_sha256"]: raise ValueError("HGCS父NTR gate SHA不匹配。")
...
def run(config_path,output_dir,expected_commit):
    ...
    if any(sha256_file(path)!=expected for path,expected in specs): raise ValueError("HGCS episode父模型或cache SHA不匹配。")
    input_sha["class_name_embeddings"]=config["class_name_embeddings_sha256"]; input_sha["att_splits"]=sha256_file(paths["att_splits"]); device=torch.device(base_config["device"])
    if device.type!="cuda" or not torch.cuda.is_available(): raise RuntimeError("HGCS episode要求CUDA。")
    ...
    try:
        ...
        for fold_id,(ps,pu) in enumerate(folds):
            ...
            package["components"]=(ff,package["scale"].to(device),fold_cra,fold_visual,beta_visual,gamma,torch.isin(allclasses,ps).to(device),name_model); packages.append(package); del fold_model
        model=HierarchicalGroupCommonSuppression(names,config["group_count"],config["max_group_beta"]).to(device); optimizer=torch.optim.Adam(model.parameters(),lr=float(config["lr"]),weight_decay=0.0); mapping=torch.full((200,),-1,dtype=torch.long); mapping[seenclasses]=torch.arange(150); generators=[torch.Generator(device="cpu").manual_seed(seed*59000+i) for i in range(3)]; half=int(config["batch_half"]); history=[]; input_sha.update(h1.verify_inputs(base_config,paths,h1.OFFICIAL_KEYS)); tensors.update({name:torch.load(paths[name],map_location="cpu",weights_only=True) for name in h1.OFFICIAL_KEYS}); initial=evaluate(main_components,model,tensors,seenclasses,unseenclasses,device); best_H=initial["H"]; best_epoch=0; best_state=copy.deepcopy(model.state_dict()); history.append({"epoch":0,"official_metrics_percent":initial,"beta":0.0}); print(f"epoch=0 official_H={best_H:.6f} beta=0")
        for epoch in range(1,int(config["epochs"])+1):
            ...
            for fold_id,package in enumerate(packages):
                ...
                for _ in range(steps):
                    gen=generators[fold_id]; si=package["seen_indices"][torch.randperm(package["seen_indices"].numel(),generator=gen)[:half]]; ui=package["unseen_indices"][torch.randperm(package["unseen_indices"].numel(),generator=gen)[:half]]; indices=torch.cat((si,ui)); images=tensors["train_features"][indices].to(device).float(); targets=mapping[labels[indices]].to(device); parent_logit=parent_logits(images,*package["components"],seenclasses); logits=model(parent_logit,images,seenclasses); loss=F.cross_entropy(logits,targets); optimizer.zero_grad(set_to_none=True); loss.backward(); optimizer.step(); loss_sum+=float(loss.detach()); count+=1
            official=evaluate(main_components,model,tensors,seenclasses,unseenclasses,device); row={"epoch":epoch,"loss":loss_sum/count,"official_metrics_percent":official,"beta":float(model.beta().detach())}; history.append(row); print(f"epoch={epoch} loss={row['loss']:.6f} official_H={official['H']:.6f} beta={row['beta']:.6f}")
            if official["H"]>best_H: best_H=official["H"]; best_epoch=epoch; best_state=copy.deepcopy(model.state_dict())
        model.load_state_dict(best_state,strict=True); torch.save({"attempt_id":config["attempt_id"],"code_commit":code_commit,"config":config,"selected_epoch":best_epoch,"hgcs_state_dict":best_state,"history":history},output_dir/"hgcs_model.pth"); parent_model=HierarchicalGroupCommonSuppression(names,config["group_count"],config["max_group_beta"]).to(device); parent_metrics=evaluate(main_components,parent_model,tensors,seenclasses,unseenclasses,device); candidate_metrics=evaluate(main_components,model,tensors,seenclasses,unseenclasses,device); delta={key:candidate_metrics[key]-float(config["parent_metrics_percent"][key]) for key in ("U","S","H","ZS")}; beta=float(model.beta().detach()); success=candidate_metrics["H"]>80.71256500221342 and delta["U"]>=-2 and delta["S"]>=-2 and -0.98*float(config["max_group_beta"])<beta<-0.1; atomic_write_json(output_dir/"data_fingerprints.json",{"files":input_sha}); metrics={"attempt_id":config["attempt_id"],"idea_id":config["idea_id"],"framework_id":config["framework_id"],"code_commit":code_commit,"config_sha256":config_sha,"base_config_sha256":base_config_sha,"evaluation_protocol":h1.EVALUATION_PROTOCOL,"test_used_for_selection":True,"unseen_images_used_for_gradient":False,"selected_epoch":best_epoch,"learned_beta":beta,"group_sizes":torch.bincount(model.assignment.cpu(),minlength=int(config["group_count"])).tolist(),"recomputed_parent_metrics_percent":parent_metrics,"parent_metrics_percent":config["parent_metrics_percent"],"candidate_metrics_percent":candidate_metrics,"delta_vs_parent_percent_points":delta,"success":success,"hgcs_model_sha256":sha256_file(output_dir/"hgcs_model.pth")}; atomic_write_json(output_dir/"metrics.json",metrics); print(metrics); return metrics
    finally:
