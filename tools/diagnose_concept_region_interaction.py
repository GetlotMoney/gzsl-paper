"""Gate concept-specific cross-region non-additive interactions for IDEA-168."""
...
def load_config(path: Path) -> dict:
    ...
    if actual != required:
        raise ValueError(
            f"IDEA-168配置错误：缺少={sorted(required-actual)}，多出={sorted(actual-required)}"
        )
    if (
        config["schema_version"] != "gzsl-paper.concept-region-interaction.v1"
...
def setup_concepts(config: dict, clip_model, device: torch.device) -> dict:
    ...
    if len(labels) != 7057 or len(seen_classes) != 150:
        raise ValueError("IDEA-168固定CUB 7,057张formal-seen图像与150类。")
    ...
    if eligible != expected or len(eligible) != 27:
        raise ValueError(f"IDEA-168没有复现固定27概念轴：{eligible}")
...
def stratified_rows(labels: np.ndarray, classes: list[int], count: int, seed: int) -> np.ndarray:
    if count % len(classes) != 0:
        raise ValueError("IDEA-168固定每个pseudo-unseen类抽取相同图像数。")
...
def _prepare_identity(args, config):
    ...
    if current_code_commit() != args.expected_commit:
        raise ValueError("IDEA-168代码commit身份错误。")
    if sha256_file(args.config) != args.expected_config_sha:
        raise ValueError("IDEA-168 config SHA错误。")
...
def prepare(args, config: dict, device: torch.device):
    ...
    result = {
        "schema_version": "gzsl-paper.concept-region-interaction-prepared.v1",
        "idea_id": "IDEA-168",
        "code_commit": args.expected_commit,
        "config_sha256": args.expected_config_sha,
        "asset_identity": asset_identity,
...
def load_prepared(path: Path, expected_commit: str, expected_config_sha: str):
    ...
    if sha256_file(checkpoint) != result["checkpoint_sha256"]:
        raise ValueError("IDEA-168 prepared checkpoint SHA错误。")
...
def intervene(args, config: dict, device: torch.device):
    ...
    result = {
        "schema_version": "gzsl-paper.concept-region-interaction-shard.v1",
        "idea_id": "IDEA-168",
        "code_commit": args.expected_commit,
        "config_sha256": args.expected_config_sha,
...
def merge(args, config: dict):
    ...
    result = {
        "schema_version": "gzsl-paper.concept-region-interaction-result.v1",
        "idea_id": "IDEA-168",
        "code_commit": args.expected_commit,
        "config_sha256": args.expected_config_sha,
        "decision": decision,
