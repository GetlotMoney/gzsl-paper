"""Gate content-aware inpainted region interactions for IDEA-170."""
...
PERTURBATIONS = ("harmonic_inpaint_64", "boundary_reflect_inpaint")
...
def load_config(path: Path) -> dict:
    ...
    required = {
        "schema_version", "idea_id", "rescue_of", "parent_raw_result_uri",
        "parent_raw_result_sha256", "parent_contrastive_result_uri",
        "parent_contrastive_result_sha256", "dataset", "role_texts", "role_texts_sha256",
        ...
        "harmonic_iterations", "boundary_context_pixels", "minimum_interaction_pairs",
    ...
    if actual != required:
        raise ValueError(
            f"IDEA-170配置错误：缺少={sorted(required-actual)}，多出={sorted(actual-required)}"
        )
    if (
        config["schema_version"] != "gzsl-paper.content-aware-inpainted-interaction.v1"
        or config["idea_id"] != "IDEA-170"
        ...
        raise ValueError("IDEA-170配置身份或数据边界错误。")
    if int(config["token_grid_side"]) != 24 or int(config["window_patch_side"]) != 4:
        raise ValueError("IDEA-170固定使用24x24 token与4x4-patch窗口。")
    if int(config["shard_count"]) != 2 or config["primary_readout"] != "fixed_original_attention":
        raise ValueError("IDEA-170固定双卡与原图Attention线性读出。")
    if config["cublas_workspace_config"] != CUBLAS_WORKSPACE_CONFIG:
        raise ValueError("IDEA-170固定cuBLAS确定性工作区为:4096:8。")
    if int(config["harmonic_iterations"]) != 64 or int(config["boundary_context_pixels"]) != 56:
        raise ValueError("IDEA-170固定64步调和补全与56像素边界上下文。")
    return config


def validate_assets(config: dict) -> dict:
    for prefix in ("parent_raw", "parent_contrastive"):
        path = Path(config[f"{prefix}_result_uri"])
        if sha256_file(path) != config[f"{prefix}_result_sha256"]:
            raise ValueError(f"{prefix}父结果SHA错误。")
    ...
    return {
        ...
        "parent_raw_result_sha256": config["parent_raw_result_sha256"],
        "parent_contrastive_result_sha256": config["parent_contrastive_result_sha256"],
    }
...
def setup_concepts(config: dict, clip_model, device: torch.device) -> dict:
    ...
    if len(labels) != 7057 or len(seen_classes) != 150:
        raise ValueError("IDEA-170固定CUB 7,057张formal-seen图像与150类。")
    ...
    if eligible != expected or len(eligible) != 27:
        raise ValueError(f"IDEA-170没有复现固定27概念轴：{eligible}")
...
def stratified_rows(labels: np.ndarray, classes: list[int], count: int, seed: int) -> np.ndarray:
    if count % len(classes) != 0:
        raise ValueError("IDEA-170固定每个pseudo-unseen类抽取相同图像数。")
...
def window_pixel_mask(
    image: torch.Tensor, windows, *, patch_pixels: int = 14, window_patch_side: int = 4,
) -> torch.Tensor:
    mask = torch.zeros(image.shape[-2:], dtype=torch.bool, device=image.device)
    for row, column in windows:
        top, left = int(row) * patch_pixels, int(column) * patch_pixels
        bottom, right = top + window_patch_side * patch_pixels, left + window_patch_side * patch_pixels
        mask[top:bottom, left:right] = True
    return mask


def harmonic_inpaint(
    image: torch.Tensor, windows, *, iterations: int = 64,
    patch_pixels: int = 14, window_patch_side: int = 4,
) -> torch.Tensor:
    mask = window_pixel_mask(
        image, windows, patch_pixels=patch_pixels, window_patch_side=window_patch_side
    )
    expanded = F.conv2d(
        mask.float().reshape(1, 1, *mask.shape),
        torch.ones(1, 1, 3, 3, device=image.device),
        padding=1,
    )[0, 0] > 0
    ...
    for _ in range(int(iterations)):
        ...
        output = torch.where(mask.unsqueeze(0), average, fixed)
    return output


def boundary_reflect_inpaint(
    image: torch.Tensor, windows, *, patch_pixels: int = 14, window_patch_side: int = 4,
) -> torch.Tensor:
    output = image.clone()
    full_mask = window_pixel_mask(
        image, windows, patch_pixels=patch_pixels, window_patch_side=window_patch_side
    )
    ...
    for row, column in windows:
        top, left = int(row) * patch_pixels, int(column) * patch_pixels
        bottom, right = top + window_patch_side * patch_pixels, left + window_patch_side * patch_pixels
        rows = torch.arange(top, bottom, device=image.device)
        columns = torch.arange(left, right, device=image.device)
        rr, cc = torch.meshgrid(rows, columns, indexing="ij")
        source_rows = torch.stack(
            (
                (2 * top - 1 - rr).clamp(0, height - 1),
                (2 * bottom - 1 - rr).clamp(0, height - 1),
                rr,
                rr,
            )
        )
        source_columns = torch.stack(
            (
                cc,
                cc,
                (2 * left - 1 - cc).clamp(0, width - 1),
                (2 * right - 1 - cc).clamp(0, width - 1),
            )
        )
        distances = torch.stack(
            (rr - top + 1, bottom - rr, cc - left + 1, right - cc)
        ).float()
        invalid = full_mask[source_rows, source_columns]
        distances = distances.masked_fill(invalid, float("inf"))
        choice = distances.argmin(dim=0, keepdim=True)
        if not torch.isfinite(distances.gather(0, choice)).all():
            raise ValueError("反射补全找不到窗口外上下文。")
        chosen_rows = source_rows.gather(0, choice)[0]
        chosen_columns = source_columns.gather(0, choice)[0]
        output[:, rr, cc] = image[:, chosen_rows, chosen_columns]
    return output


def content_aware_inpaint(image, windows, *, mode, iterations, window_patch_side):
    if mode == "harmonic_inpaint_64":
        return harmonic_inpaint(
            image, windows, iterations=iterations, window_patch_side=window_patch_side
        )
    if mode == "boundary_reflect_inpaint":
        return boundary_reflect_inpaint(
            image, windows, window_patch_side=window_patch_side
        )
    raise ValueError(f"未知内容补全：{mode}")
...
def _prepare_identity(args, config):
    ...
    if current_code_commit() != args.expected_commit:
        raise ValueError("IDEA-170代码commit身份错误。")
    if sha256_file(args.config) != args.expected_config_sha:
        raise ValueError("IDEA-170 config SHA错误。")
...
def prepare(args, config: dict, device: torch.device):
    ...
    result = {
        "schema_version": "gzsl-paper.content-aware-inpainted-interaction-prepared.v1",
        "idea_id": "IDEA-170",
        "code_commit": args.expected_commit,
        "config_sha256": args.expected_config_sha,
        "asset_identity": asset_identity,
...
def load_prepared(path: Path, expected_commit: str, expected_config_sha: str):
    ...
    if (
        result.get("schema_version") != "gzsl-paper.content-aware-inpainted-interaction-prepared.v1"
        or result.get("idea_id") != "IDEA-170"
    ...
    if (
        ...
        raise ValueError("IDEA-170 prepared JSON与checkpoint语义身份不一致。")
...
def intervene(args, config: dict, device: torch.device):
    ...
    if int(args.shard_count) != int(config["shard_count"]) or not 0 <= int(args.shard_index) < int(args.shard_count):
        raise ValueError("IDEA-170必须恰好使用配置声明的两个shard。")
    torch.manual_seed(int(config["seed"]))
    np.random.seed(int(config["seed"]))
    torch.use_deterministic_algorithms(True)
    torch.backends.cuda.matmul.allow_tf32 = False
    ...
    for train_row in shard_rows:
        ...
        logits, _, attentions = reader.details(raw_patches, concepts)
                ...
                for pair_name in ("target", "hard", "random"):
                    variants.extend(
                        pair_variants(
                            ...
                            pairs[pair_name],
                            mode,
                            int(config["harmonic_iterations"]),
                            window_side,
                        )
                    )
    ...
    result = {
        "schema_version": "gzsl-paper.content-aware-inpainted-interaction-shard.v1",
        "idea_id": "IDEA-170",
        "code_commit": args.expected_commit,
        "config_sha256": args.expected_config_sha,
...
def merge(args, config: dict):
    ...
    if len(shards) != int(config["shard_count"]):
        raise ValueError("IDEA-170 merge必须恰好接收两个shard。")
    ...
    for shard, path in zip(shards, args.shards):
        if (
            shard.get("schema_version") != "gzsl-paper.content-aware-inpainted-interaction-shard.v1"
            or shard.get("idea_id") != "IDEA-170"
    ...
    if records:
        ...
        eta_harmonic = np.asarray(
            [record["perturbations"]["harmonic_inpaint_64"]["target"]["eta"] for record in records]
        )
        eta_reflect = np.asarray(
            [record["perturbations"]["boundary_reflect_inpaint"]["target"]["eta"] for record in records]
        )
        concordance = np.sign(eta_harmonic) * np.sign(eta_reflect)
    ...
    result = {
        "schema_version": "gzsl-paper.content-aware-inpainted-interaction-result.v1",
        "idea_id": "IDEA-170",
        "code_commit": args.expected_commit,
        "config_sha256": args.expected_config_sha,
        "decision": decision,
        ...
        "perturbation_semantics": {
            "harmonic_inpaint_64": "用窗口外真实边界初始化并执行64步离散调和补全；不等同生成式反事实。",
            "boundary_reflect_inpaint": "沿最近可用窗口边界反射当前图像纹理；不使用外部图像或模型。",
        },
