...
- 单次审核前置全流程从主Agent开始准备共享证据计时，到两名Agent完成独立检查、一次交叉交流和最终双签为止，硬性不得超过15分钟；这不是优化目标，而是owner固定截止线。
- 超过15分钟，本次审核自动标记`failed_due_to_audit_timeout`，不得继续签字或启动训练；立即记录真实耗时、最慢步骤、重复劳动和可删除项，然后先瘦身流程再重新开始下一次审核。
- 默认共享证据只保留：准确diff、最小相关测试摘要、数据/test边界静态检查、一次真实GPU micro-batch和正式输出不存在证明。不得默认增加大文件全量SHA、重复整仓测试、双GPU重复预载、额外收据层或新证明文档。
- 已有manifest、已冻结资产SHA、未变化测试和父结果直接复用，不重新计算。证据不足只记录`warning_evidence_incomplete`，不因补证据拖延审核；但两名Agent仍必须明确当前已知P0/P1。
- 两名Agent必须从一开始并行；双方复用同一证据，各自完成初始清单后只交流一次。不得串行等待另一Agent结束后再启动，也不得因交流追加第二轮完整审核。
- 任一Agent可在仓库外临时目录重放必要micro-batch；正式server smoke、训练、资产发布和RUN仍必须等15分钟内完成的双Agent最终通过。
