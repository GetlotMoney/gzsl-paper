...
## 当前创新要求（owner于2026-09-01降低门槛）

- 本节优先于下文“范式级创新准入”和“三创新论文主线”中更严格的数量与级别要求。
- 最终方法只需满足以下任一条件：整体框架形成一个可辩护的新方法组合，或框架中至少一个核心点具有可辩护的新意；不再要求语义、视觉、交互三个部署模块各自都是范式创新，也不再硬性要求三个独立范式创新点。
- 其余模块允许采用公开论文中的已有方法或标准机制，但必须准确引用、说明本项目如何使用，不得把借用模块包装成原创贡献。
- 被明确声称为核心创新的点仍须核对最近工作、写清差异并接受真实对照；创新可以是窄而真实的方法级贡献，不再强制达到“范式级”才允许进入论文框架。
- 新增或修改代码仍须遵守双Agent代码审查、数据边界和实验留痕；当前性能合同不变：Full须高于准确父基线，且S/V/I同checkpoint单模块关闭各自至少降低`1.0 H`，`H=80`仍是目标而非硬通过线。
