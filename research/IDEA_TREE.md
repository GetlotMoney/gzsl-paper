...
| `learning_generalization` | 训练目标与选择规则如何迁移到未见类别 | [`IDEA-212 / One-Text Seen-Only CE`](ideas/IDEA-212_one_text_seen_only_ce.md)：排查一文本Full是否被all-class训练CE压制unseen竞争，不包装为论文创新；[`IDEA-214 / Text Relation Visual Operator`](ideas/IDEA-214_text_relation_visual_operator.md)：删除逐图Reader，验证一文本关系能否由共享identity-residual文本到视觉算子预编译 |
