...
| `learning_generalization` | 训练目标与选择规则如何迁移到未见类别 | [`IDEA-212 / One-Text Seen-Only CE`](ideas/IDEA-212_one_text_seen_only_ce.md)：排查一文本Full是否被all-class训练CE压制unseen竞争，不包装为论文创新；[`IDEA-213 / Class-Held-Out V/I Training`](ideas/IDEA-213_class_held_out_vi_training.md)：用seen内部类别留出outer CE训练V/I，不包装为论文创新 |
