...
└─ baseline_completed
   ...
   └─ paper_method_goal
      ...
      ├─ rejected_branch：IDEA-088 / CPGSD中心化patch引导dropout（中心化后仍无增益）
      ...
      ├─ supported_candidate：IDEA-092 / AGCT歧义门控跨LLM二选一（25分位两seed，最高H=78.357224）
      ├─ rejected_branch：IDEA-093 / CCTB反共识二选一（gate过稀无效果）
      ├─ rejected_branch：IDEA-094 / MAGT多源歧义门控二选一（双文本源重复）
      ├─ rejected_branch：IDEA-095 / AGPT歧义门控局部patch二选一（局部视觉tie-break无效）
      ├─ rejected_branch：IDEA-096 / GPES门控成对证据选择器（169 pair过拟合）
      ├─ supported_candidate：IDEA-097 / GWPS soft-gate加权成对选择器（两seed最高H=78.414246）
      ...
      ├─ supported_candidate：IDEA-106 / SNPS语义近邻pair选择器（top-3两seed稳定；最高观察H=78.480710）
      ├─ supported_candidate：IDEA-107 / M-SNPS互为语义近邻pair选择器（两seed稳定，最高H=78.459247）
      ├─ rejected_branch：IDEA-108 / R-SNPS互惠加权语义近邻选择器（跨seed不能替代top-3）
...
| [`IDEA-161`](ideas/IDEA-161_intermediate_patch_concept_signal.md) | revised | 只否定中间层裸余弦读取 |
| [`IDEA-162`](ideas/IDEA-162_learnable_concept_readout_probe.md) | supported signal only | 共享概念信号可学习迁移 |
| [`IDEA-163`](ideas/IDEA-163_tri_state_evidence_predicate_set.md) | rejected | 类特定离散三态五项门槛全部失败 |
| [`IDEA-164`](ideas/IDEA-164_observable_signed_evidence.md) | owner-approved Gate 1 | 候选无关可观察性与固定参考有符号证据 |
