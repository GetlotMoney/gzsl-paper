...
零初始化残差支路在epoch 0精确复现TRY-078的`H=77.572682%`，但后续20轮全部降低H，最终只能选回epoch 0。完整top-5直接重训和冻结父模型后的增量学习均未增加整体H，说明邻域排序细节不是当前瓶颈；不消耗剩余两次补救预算，IDEA-025标记`rejected`。
