...
SNPS RESCUE-2 union top-3 seed5达到`U/S/H/ZS=76.883179/80.116844/78.466710/84.121209%`；479条语义边、4458个pair和H均位于union top-5与mutual top-5之间，ZS为三种图规则最高。因H超过C-RGWPS，追加seed7检验稳定性。当前累计194组。

SNPS RESCUE-2 union top-3 seed7达到`U/S/H/ZS=76.847547/80.112571/78.446100/83.987880%`，相对C-RGWPS提高H `0.033391`且超过mutual top-5 `0.019202`。top-3两seedH范围仅`0.020610`并都高于C-RGWPS，固定为SNPS家族稳定结构；union top-5 seed5 `78.480710%`继续作为owner口径最高成绩。当前累计195组。
...
TR-RDSS seed5达到`U/S/H/ZS=76.917648/80.219018/78.533653/84.054536%`，相对SNPS父模型提高H `0.066943`。role scale weight=`-0.037190`，旧权重drift=`0.115727`，信赖域允许有限协调；追加seed7检验稳定性。当前累计205组。
...
下一实验`V2-INNOVATION-095 / S-EDPS`按Chen-style协议执行真正分阶段训练：先加载稳定SNPS top-3的训练权重，再用EDPS循环证据屏蔽继续训练全部12维selector。它只改变优化起点与顺序，推理公式和seen-only梯度边界不变。
...
S-EDPS RESCUE-1的seed7低学习率条件达到`U/S/H/ZS=76.914215/80.112571/78.480820/84.021211%`，相对同seed父模型H提高`0.034720`。追加同一`1e-4`条件seed5，避免跨参数拼接。当前累计有效实验225组。
...
V2-TUNE-002 RESCUE-1达到`H_val=76.573831%`，略高于旧baseline，但U/ZS分别下降`4.100567/1.150745`，提升由S驱动。RESCUE-2保持权重0.1，只允许inner episode更新TST-NTR与CCGR，避免pseudo-unseen图像通过共享TG-VPR被直接记忆。
...
按owner最终确认的简化方案建立`V2-TUNE-003`：完整150个seen类做纯三折，每折100类训练、50类只验证，取消pseudo-unseen额外loss和外层嵌套；首RUN评估当前三模块基准超参数，全程不加载official test。
...
owner随后冻结最终论文实验为CUB/AWA2/SUN Attribute三数据集、OpenAI CLIP ViT-L/14@336px可追溯缓存、TG-VPR→TST-NTR→CCGR三模块与Chen-style test-selected协议。三数据集通用xlsa17加载、动态类别轴、Pure CLIP/Mean8和全部模块开关、End-to-End/50-100-50 Stagewise统一训练入口已实现。三套官方原图、xlsa17、7,736句八角色原文和OpenAI CLIP图像/文本缓存均已完成独立SHA、形状、有限性与归一化校验；资产ID为CUB=`f61a4af0d7644477`、AWA2=`803b2f38aae164e5`、SUN=`baaea1e1bfdc31e4`。尚未运行official test、创建pre-run commit或产生训练结果。
