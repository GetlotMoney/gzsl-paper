...
- [ ] 审核前置全流程已启动15分钟硬计时；共享证据只含准确diff、最小相关测试、数据/test边界、一次GPU micro-batch和输出不存在证明。
- [ ] 两名Agent并行启动并只交流一次；共享证据准备到最终双签总耗时不超过15分钟。
- [ ] 若超过15分钟，本次已标记`failed_due_to_audit_timeout`、禁止运行，并记录瓶颈与删减方案后重新审核；不得超时继续补证据。
- [ ] 已有manifest/SHA/未变化测试直接复用；未补齐证据只记warning，不重复整仓测试、全量大文件SHA或双GPU预载。
