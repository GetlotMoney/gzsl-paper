...
执行`docs/LONG_HORIZON_EXPERIMENT_PLAN.md`，NG-CCGR补救1从当前最佳4维CCGR函数等价初始化8维邻域模型；不能复活已止损模块或只做参数搜索。
